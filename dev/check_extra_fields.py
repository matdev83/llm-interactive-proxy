...
from src.core.config.models.misc import UsageTrackingConfig, AssessmentConfig, ReplacementConfig, EditPrecisionConfig, CodebuffConfig
...
def check_model_schema_match(model_name: str, model_fields: set[str], schema_props: dict[str, Any], context: str = ""):
...
nested_configs = [
    ...
    ('assessment', 'AssessmentConfig', get_pydantic_fields(AssessmentConfig)),
...
for section_name, config_name, fields in nested_configs:
    if section_name in top_level_props:
        ...
        print(f"\n=== {section_name} ===")
        issues |= check_model_schema_match(config_name, fields, section_schema, section_name)
...
issues |= check_model_schema_match("ToolCallReactorConfig", tool_fields, tool_schema.get('properties', {}))
