...
def analyze_specs(specs_dir: Path) -> Tuple[List[str], List[str]]:
    ...
    spec_folders = [d for d in specs_dir.iterdir() if d.is_dir() and d.name != "archive"]
    ...
    for spec_folder in sorted(spec_folders):
        ...
        if is_completed(spec_data, spec_folder):
            ...
            impl_status = spec_data.get("implementation_status", "unknown") if spec_data else "unknown"
            marker = " (has completion marker)" if has_completion_marker_file(spec_folder) else ""
