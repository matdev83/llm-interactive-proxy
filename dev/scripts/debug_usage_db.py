...
async def inspect_db():
    ...
    async with engine.session() as session:
        ...
        count_usage = (await session.execute(select(func.count()).select_from(UsageRecordTable))).scalar()
        ...
        stmt_with_tokens = select(func.count()).select_from(SessionMetricsTable).where(SessionMetricsTable.total_tokens > 0)
        ...
        stmt = select(func.count()).select_from(SessionMetricsTable).where(SessionMetricsTable.last_activity >= start_of_day)
