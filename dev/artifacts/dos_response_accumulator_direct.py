...
def test_direct_vulnerability():
    ...
    large_payload = {
        "choices": [{"delta": {"content": "A" * (5 * 1024 * 1024)}}],  # 5MB content
        "usage": {
    ...
    print(
        f"Created SSE data line size: {payload_size:,} bytes ({payload_size/1024/1024:.2f} MB)"
    )
    ...
    try:
        ...
        print(
            f"Successfully parsed payload with {len(large_payload.get('large_array', []))} array elements"
        )
        ...
        if any(
            ...
            for keyword in ["memory", "size", "limit", "recursion"]
        ):
