...
async def test_fixed_memory_management():
    ...
    max_entries_per_session = (
        max(len(history) for history in tracker._history.values())
        if tracker._history
        else 0
    )
    ...
    is_successful = (
        (final_memory - initial_memory) < reasonable_memory_growth
        and len(tracker._history) <= 500
        and final_total_entries <= reasonable_total_entries
        and max_entries_per_session <= 50
    )
    ...
    if is_successful:
        ...
        print("Evidence:")
        print(
            f"  - Controlled memory growth: {final_memory - initial_memory:.2f} MB < {reasonable_memory_growth} MB"
        )
        print(f"  - Session limit enforced: {len(tracker._history)} <= 500")
        print(f"  - Per-session limit enforced: {max_entries_per_session} <= 50")
        print(
            f"  - Total entries controlled: {final_total_entries} <= {reasonable_total_entries}"
        )
    else:
        ...
        if (final_memory - initial_memory) >= reasonable_memory_growth:
            print(
                f"  - Memory growth still high: {final_memory - initial_memory:.2f} MB"
            )
        ...
        if final_total_entries > reasonable_total_entries:
            print(
                f"  - Total entries too high: {final_total_entries} > {reasonable_total_entries}"
            )
