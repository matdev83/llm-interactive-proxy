...
def test_extreme_case():
    ...
    print(
        f"Pattern analysis range: {tracker.max_pattern_length - tracker.min_pattern_length} pattern lengths"
    )
    ...
    theoretical_ops = (tracker.max_pattern_length - tracker.min_pattern_length) * len(
        malicious_content
    )
...
def test_repeated_attacks():
    ...
    tracker = RollingHashTracker(max_pattern_length=500, max_history=15000)
