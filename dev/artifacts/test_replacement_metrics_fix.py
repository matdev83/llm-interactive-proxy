...
def main():
    ...
    if activation_leak_fixed and opt_out_leak_fixed:
        ...
        print(f"  [OK] activation_timestamps bounded at {final_activation_count} <= {max_activation_timestamps}")
        print(f"  [OK] opt_out_timestamps bounded at {final_opt_out_count} <= {max_opt_out_timestamps}")
    else:
        print("FAILURE: Memory leak persists!")
        if not activation_leak_fixed:
            print(f"  [FAIL] activation_timestamps exceeded limit: {final_activation_count} > {max_activation_timestamps}")
        if not opt_out_leak_fixed:
            print(f"  [FAIL] opt_out_timestamps exceeded limit: {final_opt_out_count} > {max_opt_out_timestamps}")
    print("=" * 80)
