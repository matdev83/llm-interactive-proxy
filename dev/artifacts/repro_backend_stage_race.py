...
async def add_cleanup_tasks_concurrently():
    """Simulate concurrent additions to _cleanup_tasks set."""
    ...
    print(f"Actual tasks in _cleanup_tasks: {len(stage._cleanup_tasks)}")

    # Check for missing tasks
    missing = len(created_tasks) - len(stage._cleanup_tasks)
    if missing > 0:
