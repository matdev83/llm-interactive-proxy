...
async def main():
    ...
    for i in range(num_sessions):
        ...
        if (i + 1) % 100 == 0:
            ...
            print(
                f"  Sessions processed: {i + 1}, Memory growth: {memory_growth:.2f} MB"
            )
            print(
                f"  _reasoning_extracted size: {len(processor._reasoning_extracted)} entries"
            )
            print(
                f"  _streaming_buffers size: {len(processor._streaming_buffers)} entries"
            )
    ...
    all_empty = (
        len(processor._reasoning_extracted) == 0
        and len(processor._streaming_buffers) == 0
        and len(processor._stream_states) == 0
    )
