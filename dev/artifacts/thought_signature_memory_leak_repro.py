...
def simulate_tool_calls(manager: ThoughtSignatureManager, count: int, session_id: str) -> None:
...
async def main():
    ...
    for batch in range(5):
        ...
        print(f"Batch {batch + 1}: {current_memory} KB, "
        ...
        for i in range(100):
            manager.store_signatures_from_tool_calls([{
                "id": f"temp_call_{i}",
                "extra_content": {"google": {"thought_signature": f"temp_sig_{i}"}}
            }], f"temp_session_{i}")
    ...
    print(f"After session clear: {after_clear_memory} KB, "
