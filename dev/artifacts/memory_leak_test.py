...
async def test_memory_growth():
    ...
    tracker = InMemoryToolCallHistoryTracker(
        session_ttl_seconds=60, max_sessions=1000  # 1 minute TTL
    )
    ...
    for session_id, history in tracker._history.items():
        ...
        if len(history) > 1000:
            print(
                f"Session {session_id} has {len(history)} entries (exceeds 1000 limit!)"
            )
    ...
    is_leaked = (
        ...
        or len(tracker._history) > 50  # Should be mostly cleared after TTL
        or any(len(history) > 1000 for history in tracker._history.values())
    )
    ...
    if is_leaked:
        ...
        if (final_memory - initial_memory) > 50:
            print(
                f"  - Excessive memory growth: {final_memory - initial_memory:.2f} MB"
            )
