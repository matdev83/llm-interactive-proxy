...
def test_security_limits():
    ...
    try:
        ...
        if "error" in metadata:
            ...
            print(f"   [OK] Safe JSON accepted (depth: {SSEDecoder.MAX_JSON_DEPTH - 1})")
    except Exception as e:
    ...
    try:
        ...
        if "error" in metadata:
            ...
            print(f"   [OK] Lines limit handled (processed {len(result[0]) if isinstance(result[0], str) else 'parsed object'})")
    except Exception as e:
        print(f"   [PROTECTED] Exception caught: {type(e).__name__}")
