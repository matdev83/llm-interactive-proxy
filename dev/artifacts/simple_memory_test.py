...
async def test_memory_growth():
    ...
    tracker = TestToolCallHistoryTracker(
        session_ttl_seconds=10, max_sessions=1000  # 10 seconds TTL for quick testing
    )
    ...
    print(
        f"Creating {num_sessions} sessions with {calls_per_session} tool calls each..."
    )
    ...
    for session_id in range(num_sessions):
        ...
        for call_id in range(calls_per_session):
            context = {
                ...
                "tool_arguments": {
                    "arg1": f"value_{call_id}",
                    "arg2": f"data_{call_id}" * 10,
                },
            }
    ...
    for session_id, history in tracker._history.items():
        ...
        if entries_count > 1000:
            print(
                f"Session {session_id} has {entries_count} entries (exceeds 1000 limit!)"
            )
    ...
    is_leaked = (
        (final_memory - initial_memory) > expected_max_memory
        or len(tracker._history) > 100  # Should be mostly cleared after TTL
        or total_entries > expected_max_total_entries * 0.9  # Close to theoretical max
    )
    ...
    if is_leaked:
        ...
        if (final_memory - initial_memory) > expected_max_memory:
            print(
                f"  - Excessive memory growth: {final_memory - initial_memory:.2f} MB (expected < {expected_max_memory} MB)"
            )
        if len(tracker._history) > 100:
            print(
                f"  - Sessions not cleaned up: {len(tracker._history)} remaining (TTL = {tracker._session_ttl_seconds}s)"
            )
        if total_entries > expected_max_total_entries * 0.9:
            print(
                f"  - Too many total entries: {total_entries} (max should be ~{expected_max_total_entries})"
            )
            print(
                "  - ISSUE: Per-session limit of 1000 is insufficient across many sessions!"
            )
    else:
