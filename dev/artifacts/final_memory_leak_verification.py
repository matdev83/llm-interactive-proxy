        ...
        for hour in range(8):  # 8 hours of activity per day
            ...
            for i, ctx in enumerate(current_hour_connections):
                if i % 5 != 0:  # 80% complete normally
                    ...
                    tracker.increment_rx(
                        f"session-{connection_counter - batch_size + i}",
                        "openai.gpt4",
                        100,
                    )
            ...
            for i in range(abandoned_count):
                ...
                if idx < len(current_hour_connections):
                    ...
                    activity.started_at = (
                        time.time() - 60
                    )  # 1 minute old (older than some intervals)
            ...
            current_memory = sys.getsizeof(tracker._connections) + sum(
                ...
                for k, v in tracker._connections.items()
            )
            memory_samples.append(current_memory)

            print(
                f"  Hour {hour + 1}: {tracker.get_connection_count()} active connections, "
