#!/usr/bin/env python3
"""
Simple test to verify XML DoS vulnerability is fixed.
"""
...
<lolz>&lol4;</lolz>"""
    ...
    try:
        ...
        if "xml_entity_expansion" in str(e.details):
            print("[PASS] XML bomb blocked (entity expansion detected)")
            return True
        else:
            print(f"[FAIL] XML bomb blocked for wrong reason: {e.details}")
            return False
...
<samlp:Response xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol">
  <saml:Assertion xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion">
    ...
    try:
        ...
        print(f"[FAIL] Legitimate XML blocked: {type(e).__name__}: {e}")
        return False
...
if __name__ == "__main__":
    ...
    print("SUMMARY:")
    if test1 and test2:
        print("[SUCCESS] XML DoS vulnerability is FIXED!")
        print("- XML bomb attacks blocked")
