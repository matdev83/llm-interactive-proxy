...
def create_large_json_payload(multiplier=10):
    ...
    for i in range(200000 * multiplier):  # 200k elements * multiplier
        large_array.append({
            ...
            "nested": {
                "level1": {"level2": {"level3": {"level4": {"level5": "very deep nesting test data"}}}}
            }
        })
    ...
    for multiplier in multipliers:
        ...
        for vector_name, content in attack_vectors:
            ...
            try:
                ...
                if duration > 1.0:
                    print(f"  [WARNING] Processing took {duration:.2f} seconds - possible issue")
                else:
                    print(f"  [SUCCESS] Processing was fast ({duration:.2f}s) - protection working!")
