...
def test_reasoning_chunks_limit():
    ...
    for i in range(num_chunks):
        ...
        if (i + 1) % 500 == 0:
            print(
                f"  Added {i + 1} chunks, deque length: {len(state.reasoning_chunks)}"
            )

    print(f"\nFinal deque length: {len(state.reasoning_chunks)}")
    assert (
        len(state.reasoning_chunks) <= 1000
    ), f"Expected max 1000 chunks, got {len(state.reasoning_chunks)}"
...
def test_detected_calls_limit():
    ...
    assert (
        len(state.detected_calls) <= 1000
    ), f"Expected max 1000 calls, got {len(state.detected_calls)}"
...
def test_extracted_tool_calls_limit():
    ...
    for i in range(num_calls):
        ...
        if (i + 1) % 500 == 0:
            print(
                f"  Added {i + 1} calls, list length: {len(state.extracted_tool_calls)}"
            )

    print(f"\nFinal list length: {len(state.extracted_tool_calls)}")
    assert (
        len(state.extracted_tool_calls) <= 1000
    ), f"Expected max 1000 calls, got {len(state.extracted_tool_calls)}"
