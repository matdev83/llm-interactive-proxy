...
def test_fix():
    ...
    assert len(registry._states) == 1, f"Expected 1 state after cleanup, got {len(registry._states)}"
    ...
    assert len(registry._states) == 11, f"Expected 11 states, got {len(registry._states)}"
    ...
    assert len(registry._states) == 1, f"Expected 1 state after cleanup, got {len(registry._states)}"
    assert "test-stream-0" in registry._states, "test-stream-0 should be the remaining state"
