...
async def test_memory_leak_fix():
    ...
    if final_count == 0:
        ...
        print(
            f"   - {len(connection_batches) - final_count} connections automatically cleaned up"
        )
        ...
        print(
            f"   - {final_count} connections still active out of {len(connection_batches)}"
        )
