...
def test_memory_management():
    ...
    print(f"After storing 200 entries: cache={len(manager._cache)}, secondary={len(manager._by_tool_call)}")
    
    # Cache should be limited to max_cache_size
    assert len(manager._cache) <= 100, f"Cache size {len(manager._cache)} exceeds limit of 100"
    ...
    print(f"After TTL wait + new entry: cache={len(manager._cache)}, secondary={len(manager._by_tool_call)}")
    ...
    print(f"After anonymous entries: cache={len(manager._cache)}, secondary={len(manager._by_tool_call)}")
    
    # Clear anonymous entries
    cleared = manager.clear_all_anonymous()
    print(f"After clearing anonymous: cache={len(manager._cache)}, secondary={len(manager._by_tool_call)}, cleared={cleared}")
    ...
    for i in range(20):
        session_tool_calls.append({
            ...
            "extra_content": {
                "google": {
                    "thought_signature": f"session_sig_{i}"
                }
            }
        })
    ...
    print(f"Session cleanup: before={cache_before}, after={cache_after}, cleared={cleared}")
...
def test_command_parser_cache():
    ...
    for prefix in prefixes:
        parser.command_prefix = prefix  # This should trigger pattern compilation and caching
        
    print(f"Pattern cache size after using {len(prefixes)} prefixes: {len(parser._pattern_cache)}")
