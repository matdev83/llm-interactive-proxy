...
class TestStageImpl(ValidatedTestStage):
    ...
    async def _register_services(self, services: ServiceCollection, config) -> None:
        ...
        registered_count = len(
            [v for v in self._registered_services.values() if v is not None]
        )
        ...
        ), f"Expected at most 100 registrations, got {registered_count}"

        # If significantly fewer than 100, race condition exists
        if registered_count < 95:
            print(
                f"RACE CONDITION DETECTED: Only {registered_count}/100 registrations survived"
            )
            return True
