        ...
        large_array.append(
            {
                ...
                "metadata": {"line_number": i, "type": "code_line", "modified": True},
            }
        )
...
def test_legitimate_large_payloads():
    ...
    for size_mb in sizes_mb:
        ...
        try:
            ...
            if duration > 5.0:
                print(
                    f"[WARNING] Processing took {duration:.2f} seconds - may indicate performance issue"
                )
            else:
    ...
    try:
        ...
        duration = end_time - start_time
        print(
            f"[ERROR] 10MB boundary failed after {duration:.2f} seconds: {type(e).__name__}: {e}"
        )
    ...
    try:
        ...
        if duration < 1.0 and not result:
            ...
            print(
                f"[WARNING] >10MB payload took {duration:.2f} seconds - protection may be insufficient"
            )
        ...
        duration = end_time - start_time
        print(
            f"[INFO] Exception for >10MB payload after {duration:.3f}s: {type(e).__name__}"
        )
