    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        for line_num, line in enumerate(lines, 1):
            if pattern.search(line):
                ...
                if any(skip in line.lower() for skip in ['test', 'assert', 'mock', 'fixture']):
                    continue
        ...
        for line_num, line in enumerate(lines, 1):
            if pattern.search(line):
                ...
                has_tracking = any(
                    ...
                    for keyword in ['await', 'add_background_task', 'track', 'done_callback']
                )
...
def main():
    ...
    for finding in findings:
        ...
        if finding['unbounded_dicts']:
            print(f"  [WARN] Unbounded dictionaries ({len(finding['unbounded_dicts'])}):")
            for line_num, line in finding['unbounded_dicts'][:5]:
                print(f"     Line {line_num}: {line[:60]}")
        
        if finding['unbounded_appends']:
            print(f"  [WARN] List append without bounds ({len(finding['unbounded_appends'])}):")
            for line_num, line in finding['unbounded_appends'][:5]:
                print(f"     Line {line_num}: {line[:60]}")
        
        if finding['event_subscriptions']:
            print(f"  [WARN] Event subscriptions without cleanup ({len(finding['event_subscriptions'])}):")
            for line_num, line in finding['event_subscriptions'][:5]:
                print(f"     Line {line_num}: {line[:60]}")
        
        if finding['async_tasks']:
            print(f"  [WARN] Async tasks without tracking ({len(finding['async_tasks'])}):")
            for line_num, line in finding['async_tasks'][:5]:
                print(f"     Line {line_num}: {line[:60]}")
        
        if finding['generators']:
            print(f"  [WARN] Generators without consumption ({len(finding['generators'])}):")
