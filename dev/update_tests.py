...
content = re.sub(r'assert backend == (.+)', r'assert result.backend_type == \1', content)
...
content = re.sub(
    r'assert params == ({[^}]+})',
    r'assert result.uri_params == \1',
    content
)

# Write back
with open(r"tests\unit\core\domain\test_model_utils_uri.py", "w", encoding="utf-8") as f:
    f.write(content)
