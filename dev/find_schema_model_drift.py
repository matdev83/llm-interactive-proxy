...
def main():
    ...
    if issues:
        ...
        for model_name, schema_path, section, model_type, missing, extra in issues:
            print(f"{model_name} ({section} in {schema_path}):")
            if missing:
                print(f"  MISSING in schema: {sorted(missing)}")
            if extra:
                print(f"  EXTRA in schema: {sorted(extra)}")
            print()
    else:
