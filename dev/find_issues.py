...
def find_issues(directory):
    ...
    for py_file in Path(directory).rglob("*.py"):
        ...
        if any(
            x in str(py_file)
            for x in ["test", "fixture", "mock", "stub", "dev/", "nul"]
        ):
            continue
        ...
        fixed_files = [
            ...
            "domain/translators/responses/response.py",
            "domain/translation_utils/tool_utils.py",
            "domain/commands/loop_detection_commands/loop_detection_command.py",
            "app/middleware/content_rewriting_middleware.py",
            "app/middleware_config.py",
            "app/stages/processor.py",
            "app/stages/test_stages.py",
            "app/controllers/anthropic_controller.py",
            "app/controllers/responses_controller.py",
            "config/parameter_resolution.py",
            "config/edit_precision_temperatures.py",
            "config/sources/backend_instances.py",
            "config/sources/yaml_file.py",
            "cli.py",
            "cli_support/privilege_checker.py",
            "commands/service.py",
            "services/cbor_wire_capture_service.py",
            "services/edit_precision_response_middleware.py",
            "services/angel_prompt_loader.py",
            "services/assessment_prompt_loader.py",
            "auth/sso/sso_service.py",
            "core/simulation/client_simulator.py",
            "di/registration_helpers/request_processing/_rp_orchestration_core.py",
            "services/test_execution_reminder/eos_subscriber.py",
            "loop_detection/streaming.py",
            "gemini_base/credential_coordinator.py",
            "gemini_base/credential_providers/sqlite_provider.py",
            "services/production_concurrency_guard.py",
            "openai.py",
            "openai_codex/executor.py",
        ]
            ...
            for i, line in enumerate(lines, 1):
                ...
                if re.search(r"except\s+\w+(\s+as\s+\w+)?:", line):
                    ...
                    if (
                        ...
                        if not re.search(
                            r"logger\.(error|warning|debug|info)", line
                        ) and not re.search(r"raise ", line):
                            in_except_block = False
                            except_var = None
...
if __name__ == "__main__":
    ...
    for issue in issues:
        print(
            f"{issue['file']}:{issue['line']} - {issue['type']} - {issue['context'][:80]}"
        )
