    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        async with client.stream(
            ...
            if response.status_code != 200:
                ...
                raise Exception(f"Reasoning model request failed: {response.status_code} - {error_text.decode()}")
...
async def call_execution_model(prompt: str, reasoning_output: str) -> str:
    ...
    print(f"{Colors.BLUE}System message length: {len(augmented_messages[0]['content'])} chars{Colors.END}")
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        async with client.stream(
            ...
            if response.status_code != 200:
                error_text = await response.aread()
                raise Exception(f"Execution model request failed: {response.status_code} - {error_text.decode()}")
...
async def main(prompt: str) -> None:
    ...
    try:
        ...
        print_info("Reasoning length", f"{len(reasoning_output)} characters", Colors.CYAN)
        print_info("Execution length", f"{len(execution_output)} characters", Colors.BLUE)
