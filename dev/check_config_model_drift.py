...
def check_model_vs_schema(model_class, schema_path, schema_section):
    ...
    schema_props = schema.get("properties", {}).get(schema_section, {}).get("properties", {})
    if not schema_props:
        schema_props = schema.get("properties", {}).get(schema_section, {})
    
    model_fields = set(model_class.model_fields.keys()) if hasattr(model_class, 'model_fields') else set()
...
def main():
    checks = [
        ...
        (EditPrecisionConfig, "config/schemas/app_config.schema.yaml", "edit_precision"),
    ...
    if not missing and not extra: print(f"  [OK] Fields match")
