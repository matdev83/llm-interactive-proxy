...
- `CLINE_DIR`: Path to the Cline configuration directory (optional).
- `CLINE_SECRETS_PATH`: Path to the `secrets.json` file containing auth tokens (optional).
- `CLINE_ENVIRONMENT`: Target environment (default: `production`).
- `CLINE_API_BASE_URL`: Override for the API base URL.
...
2.  **Restricted Access**: The use of the `--enable-cline-backend-debugging-override` CLI flag is strictly reserved for the project's **developers, contributors, and maintainers**. Its sole purpose is debugging and maintaining the proxy's compatibility features.
