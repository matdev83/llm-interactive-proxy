# Gemini OAuth Antigravity Backend (Internal Use Only)
...
The `gemini-oauth-antigravity` backend allows the proxy to use both Gemini and Claude models via the Antigravity sandbox environment. It shares authentication credentials with the Antigravity VS Code extension.
...
```bash
python -m src.core.cli --default-backend gemini-oauth-antigravity --enable-antigravity-backend-debugging-override
```
...
By using this proxy with the Gemini OAuth Antigravity backend configuration, you acknowledge and agree to the following terms, which constitute a binding arrangement between you and the authors of this project:
...
**If you do not agree to these terms, do not use the Gemini OAuth Antigravity backend or the debugging override flag.**
