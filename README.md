...
The Angel verification system is a proxy-level quality control feature that uses a secondary LLM to review and verify assistant responses before they reach the user. Unlike the LLM Assessment System which monitors conversation patterns over time, Angel performs immediate verification of individual responses, acting as a safety net to catch errors, logical mistakes, and problematic tool calls in real-time.
...
To customize Angel's behavior:

1. Edit the appropriate markdown file in `config/prompts/angel_prompts/`
2. Modify the instructions, add/remove problem patterns, or adjust the tone
3. Restart the proxy to load the updated prompts

**Example customization** - Add a new problem pattern to detect:

Edit `config/prompts/angel_prompts/angel_prompt.md` and add under "Problems you should look for:":
...
```bash
# Basic setup with OpenAI mini model for verification
python -m src.core.cli \
