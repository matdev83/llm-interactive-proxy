...
- [DI Enforcement Summary](docs/DI_ENFORCEMENT_SUMMARY.md): Summary of DI enforcement system simplification
...
The project enforces dependency injection patterns through runtime validation. When developing new features or modifying existing code, follow these guidelines:
...
3. **Use the `BaseCommand` validation**
   ...
   - This ensures the command was properly instantiated with all required dependencies
...
These guidelines are enforced by runtime checks that validate commands have all required dependencies properly injected.
