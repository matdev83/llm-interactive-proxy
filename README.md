...
- **OpenAI Responses** - `/v1/responses` (supported routed backends for this surface: **OpenAI** / `openai-responses`, **OpenAI Codex** backends whose `backend_type` starts with `openai-codex`, **Anthropic**, **Google Gemini** / `google`, the in-process **mock** backend used in tests, and the dedicated **Cursor CLI ACP** compatibility path). Cursor ACP rejects client-supplied Responses tools, requires one explicit absolute trusted workspace per backend instance, and is not a Codex-governed Executor provider: the Cursor runtime can still invoke its own tools inside that workspace. Restrict it to Planner/Advisor roles in a dedicated disposable or otherwise constrained workspace. Cursor catalog routes include the full backend instance name so project-bound workspaces cannot be selected by generic round-robin routing. Other backends receive an explicit `provider_limitation` error until a dedicated path exists. Multi-turn `previous_response_id` linkage uses complete visible transcript replay in an in-memory store per process; it does not retain or persist Cursor ACP process identities and does not cross worker/process boundaries—see the operator playbook under `.kiro/specs/responses-api-frontend-compliance/OPERATOR_LIVE_VERIFICATION_10.4.md` for multi-worker limitations.
...
- [OpenAI Codex](docs/user_guide/backends/openai-codex.md) — the `openai-codex`, `openai-codex-v2` and `openai-codex-app-server` variants share an **auto-discovered model catalog**: at startup the proxy runs `codex debug models` and falls back to a shipped snapshot (`src/resources/codex/codex_model_catalog.json`) on failure. No model slugs are hardcoded in the connectors. **Note (backward-incompatible):** legacy Codex slugs the CLI no longer advertises (e.g. `gpt-5.1-codex`, `gpt-5-codex`, `gpt-oss-120b`) are no longer routable — see the backend doc for the `model_catalog` override. For `openai-codex` / `openai-codex-v2`, the first N session turns (default 5) force `temperature=1` and `verbosity=high` on the resolved request by default (ChatGPT Codex rejects `temperature` on the wire, so outbound effect is `text.verbosity=high`); opt out via `extra.codex.early_session_verbosity_bump.enabled: false`.
...
- [Alibaba Token Plan (International)](docs/user_guide/backends/alibaba-token-plan-intl.md) — `alibaba-token-plan-intl` uses the Anthropic-compatible Messages API, reads `ALIBABA_TOKEN_PLAN_API_KEY` only from the environment, and dynamically publishes the plan's available model catalog
...
```
If your client already knows the exact request token count, send `extra_body.request_context_tokens`
to override heuristic token estimation for `max_context` checks on that request.
