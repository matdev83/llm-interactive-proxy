...
async def test_cache_leak():
    ...
    for i in range(2000):  # Create 2000 unique payloads
        ...
        if i % 100 == 0:
            ...
            if cache_size > service._cache_max_size:
                print(f"⚠️  LEAK DETECTED: Cache size {cache_size} exceeds limit {service._cache_max_size}")
                return True
    ...
    if final_cache_size > service._cache_max_size:
        print(f"❌ MEMORY LEAK CONFIRMED: Cache grew to {final_cache_size}, limit is {service._cache_max_size}")
        return True
    else:
