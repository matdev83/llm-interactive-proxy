...
access_pattern = re.compile(r"(?P<var>[A-Za-z_][A-Za-z0-9_]*)\s*\.\s*app\s*\.\s*state\s*\.\s*(?P<attr>[A-Za-z0-9_]+)_backend\b")
assign_pattern = re.compile(r"(?P<prefix>\s*)(?P<var>[A-Za-z_][A-Za-z0-9_]*)\s*\.\s*app\s*\.\s*state\s*\.\s*(?P<attr>[A-Za-z0-9_]+)_backend\s*=\s*(?P<expr>.+)")
...
for fp in TESTS.rglob('*.py'):
    ...
    if count_assign or count_access:
        ...
        text = ensure_import(text, 'from src.core.interfaces.backend_service_interface import IBackendService')
        fp.write_text(text, encoding='utf-8')
        modified_files += 1
        print(f"Patched {fp} (assigns={count_assign}, accesses={count_access}) -> backup {bak}")
