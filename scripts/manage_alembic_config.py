...
def manage_alembic_config(alembic_args: list[str]) -> None:
    ...
    parser = argparse.ArgumentParser(description="Manage Alembic configuration and run commands.")

    parser.add_argument("--project-root", type=str, help="Explicitly set the project root directory.")
    ...
    if not alembic_ini_path.exists():
        ...
        if alembic_example_ini_path.exists():
            ...
            print(f"INFO: alembic.ini not found. Copying from {alembic_example_ini_path}", flush=True)
            ...
            print(f"ERROR: Alembic configuration files missing. Expected: {alembic_ini_path} or {alembic_example_ini_path}", flush=True)

            print("Please create an alembic.ini file or provide an alembic.example.ini template.", flush=True)
    ...
    if should_run_alembic:
        ...
        if result.returncode != 0:
            ...
            print(f"ERROR: Alembic command failed with exit code {result.returncode}", flush=True)
