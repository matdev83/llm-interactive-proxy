...
def test_whitespace_content():
    ...
    print(f"   content.strip(): {content.strip()!r}")
    ...
    print(
        f"   StreamingContent.is_empty (explicit False): {streaming_content.is_empty}"
    )
    print(f"   StreamingContent.content: {streaming_content.content!r}")
    ...
    print(
        f"   StreamingContent.is_empty (auto-computed): {streaming_content_auto.is_empty}"
    )
    ...
    for tc in test_contents:
        ...
        print(f"   Content {tc!r}: is_empty={sc.is_empty}")
    ...
    print(f"   Input content: {whitespace_content.content!r}")
    ...
    if output_bytes.startswith(b"data: "):
        ...
        print(f"   Parsed delta.content: {delta_content!r}")
        if delta_content == "\n":
    ...
    if b'"content": "\\n"' in dict_output or b'"content":"\\n"' in dict_output:
        ...
        print(f"   Parsed delta.content: {dc!r}")
        if dc == "\n":
            print("   SUCCESS: Newline preserved!")
        else:
            print("   FAIL: Content not preserved properly")
