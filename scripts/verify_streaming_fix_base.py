...
class TransformationAnalyzer:
    ...
    def compare_backend_to_client(
        ...
        for _i, (backend, client) in enumerate(
            zip(backend_entries, client_entries, strict=False)
        ):
...
def main() -> int:
    ...
    if args.verify_usage:
        ...
        print(
            f"Usage not in content: {len(results) - len(failures)}/{len(results)} passed"
        )
        for f in failures:
            print(f"  FAIL: {f}")
        ...
        failures = [r for r in results if not r.passed]
        print(
            f"Usage at top level: {len(results) - len(failures)}/{len(results)} passed"
        )
        for f in failures:
            print(f"  FAIL: {f}")
    ...
    if args.compare:
        ...
        for comp in comparisons:
            if comp.has_differences:
                print(
                    f"Entry {comp.entry_indices}: {len(comp.differences)} differences"
                )
