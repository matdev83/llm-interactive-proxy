...
def create_openai_chunk(
    content: str, chunk_id: str = "gen-123"
) -> CanonicalStreamChunk:
...
async def simulate_backend_stream(
...
async def simulate_stream_as_sse_bytes(
...
async def simulate_wrap_inbound_stream(
...
async def simulate_wrap_outbound_stream(
    stream: AsyncIterator[bytes],
) -> AsyncIterator[bytes]:
