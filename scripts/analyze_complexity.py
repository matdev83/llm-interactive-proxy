...
def analyze_file(file_path: Path):
    ...
    try:
        ...
        for block in cc_results:
            if hasattr(block, 'complexity'):
                block_type = 'class' if isinstance(block, Class) else ('method' if (isinstance(block, Function) and block.is_method) else 'function')
...
def main():
    ...
    results_with_complexity.sort(key=lambda x: (
        -x['max_complexity'],
        -x['total_complexity'],
        -x['lines']
    ))
    ...
    print("TOP 20 MOST COMPLEX FILES (with actual complexity)")
    print("=" * 100)
    print(f"{'File':<60} {'Lines':<8} {'Max CC':<8} {'Total CC':<10} {'Funcs':<8} {'Avg CC':<8} {'MI':<8}")
    print("-" * 100)
    
    for i, result in enumerate(results_with_complexity[:20], 1):
        print(f"{result['file']:<60} {result['lines']:<8} {result['max_complexity']:<8} "
    ...
    print("=" * 100)
    print(f"{'File':<60} {'Lines':<8} {'LLOC':<8} {'SLOC':<8} {'Max CC':<8} {'Total CC':<10}")
    print("-" * 100)
    
    for i, result in enumerate(results_by_lines[:20], 1):
        print(f"{result['file']:<60} {result['lines']:<8} {result['lloc']:<8} "
    ...
    for i, result in enumerate(results_with_complexity[:5], 1):
        print(f"\n{i}. {result['file']}")
        print(f"   Lines: {result['lines']}, LLOC: {result['lloc']}, SLOC: {result['sloc']}")
        print(f"   Max Complexity: {result['max_complexity']}, Total: {result['total_complexity']}")
        print(f"   Functions: {result['function_count']}, Avg Complexity: {result['avg_complexity']:.2f}")
        print(f"   Maintainability Index: {result['maintainability_index']:.2f}")
        
        # Show top 5 most complex functions/methods
        complex_blocks = sorted(result['complexity_blocks'], key=lambda x: x['complexity'], reverse=True)
        print(f"   Top complex functions:")
        for block in complex_blocks[:5]:
            print(f"     - {block['type']} {block['name']} (line {block['lineno']}): complexity {block['complexity']}")
