...
async def test_thinking_model_availability():
    ...
    try:
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"Model '{thinking_model}' availability: {'YES' if is_available else 'NO'}")
        logger.info(f"{'='*60}")
        ...
        if is_available:
            ...
            logger.info(f"Attempting validation anyway (may fail)...")
            try:
                backend.validate_model(thinking_model)
                logger.info(f"Validation passed (using hardcoded fallback list)")
                return True
            except BackendError as e:
                logger.error(f"Validation failed: {e.message}")
                return False
