...
async def test_streaming_pipeline():
    ...
    for i, chunk in enumerate(backend_chunks[:-1]):  # Skip done chunk for this test
        ...
        print(f"   Chunk {i}: content={repr(delta_content)}, enriched_type={type(enriched).__name__}")
    ...
    for i, chunk in enumerate(backend_chunks):
        ...
        delta_content = enriched.get("choices", [{}])[0].get("delta", {}).get("content")
        print(
            f"   Chunk {i}: content={repr(delta_content)}, "
    ...
    async for sse_bytes in assembler.assemble_stream(stream_generator(), format="sse"):
        ...
        if text.strip().startswith("data: {"):
            try:
                ...
                print(f"   Output chunk: content={repr(delta_content)}")
            except Exception:
                print(f"   Output chunk: {repr(text[:100])}")
    ...
    for chunk in backend_chunks:
        delta_content = chunk.content.get("choices", [{}])[0].get("delta", {}).get("content", "")
    ...
    if input_text == output_text:
        ...
        print(f"   Missing: {repr(set(input_text) - set(output_text))}")
