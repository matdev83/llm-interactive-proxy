...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sunset-date",
        type=str,
        default=(datetime.now().replace(month=datetime.now().month + 3)).strftime(
            "%Y-%m-%d"
        ),
        ...
        try:
            ...
            for i in range(line_num, min(line_num + 50, len(content))):
                if "return " in content[i] and (
                    "Response" in content[i]
                    or "JSONResponse" in content[i]
                    or "return {" in content[i]
                ):
                    response_line = i
                    break
            ...
            indentation = len(content[response_line]) - len(
            ...
            if (
                "return {" in content[response_line]
                or "return json" in content[response_line].lower()
            ):
                # JSON response - add warning to the dict
                new_response_line = (
                    f"{indent}# DEPRECATED: This endpoint will be removed on {sunset_date}\n"
                    f"{indent}response_data = {content[response_line].removeprefix('return ').strip()}\n"
                ...
                content[response_line] = (
                    f"{indent}# Original: {content[response_line].strip()}\n"
                )
                ...
                warning_lines = [
                    f"{indent}# DEPRECATED: This endpoint will be removed on {sunset_date}\n",
                    f"{indent}response = {content[response_line].removeprefix('return ').strip()}\n",
            ...
            print(
                f"Applied deprecation warning to {endpoint['route']} in {endpoint['file']}"
            )
