...
class ImportVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        ...
        for name in node.names:
            if (name.name == self.target_module or 
                name.name == self.simple_name or
                name.name.startswith(f"{self.target_module}.") or
                (self.target_module.startswith('src.') and name.name == self.target_module[4:])):
                self.imports.append((node.lineno, f"import {name.name}"))
        self.generic_visit(node)
        
    def visit_ImportFrom(self, node):
        """Visit ImportFrom nodes."""
        if (node.module == self.target_module or 
            node.module == self.simple_name or
            (node.module and node.module.startswith(f"{self.target_module}.")) or
            (self.target_module.startswith('src.') and node.module == self.target_module[4:]) or
            (node.module and self.target_module.startswith('src.') and node.module == self.target_module[4:].split('.')[0])):
