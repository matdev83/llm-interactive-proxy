        ...
        if node.module is not None:  # Ignore relative imports with no module
            for name in node.names:
                if node.level == 0:  # Absolute import
                    ...
                    self.imports.append(
                        (
                            f"relative.{node.level}.{node.module}.{name.name}",
                            name.asname,
                        )
                    )
        self.generic_visit(node)
...
def analyze_imports(
    directory: str | Path, legacy_prefixes: list[str] | None = None
) -> dict[str, list[str]]:
    ...
    for path in directory_path.rglob("*.py"):
        if path.is_file():
            ...
            legacy_imports = [
                imp for imp, _ in imports if is_legacy_module(imp, legacy_prefixes)
            ]
...
def generate_markdown_report(
    dependencies: dict[str, list[str]], graph: dict[str, set[str]]
) -> str:
    ...
    if graph:
        ...
        lines.append(
            f"- **Most Imported Legacy Module**: {most_imported[0]} (imported by {len(most_imported[1])} modules)"
        )
