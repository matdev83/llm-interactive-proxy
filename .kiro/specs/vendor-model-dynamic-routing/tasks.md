...
- [x] 1. Align canonical model parsing and resolution boundaries with the refactored architecture
- [x] 1.0 Create a new `dynamic-routing` git branch
- [x] 1.1 Enforce `:`-only backend selection parsing in shared model utilities and resolver entry points
...
- [x] 1.2 Apply parsing and selector validation consistently across explicit-backend features
...
- [x] 1.3 Parse URI-like model-selector parameters as first-class routing inputs
...
- [x] 2. Extend resolver-centric dynamic routing for all request variants
- [x] 2.1 Route `backend:model` via deterministic round robin across eligible instances
...
- [x] 2.2 Route model-only identifiers (`model`, `vendor/model`) through capability-driven candidate selection
...
- [x] 2.3 Ensure no-backend-available outcomes fail before connector invocation
  - Return structured routing errors when all candidates are filtered as unavailable.
  - _Requirements: 2.4, 6.3_

- [x] 2.4 Introduce one shared routing entry point for outbound inference target resolution
...
- [x] 2.5 Enforce shared resolver usage across all protocol ingress surfaces
...
- [x] 2.6 Implement URI-parameter inheritance and precedence during dispatch
...
- [x] 2.7 Implement user-configurable preference-policy ranking for model-only routing
...
- [x] 3. Implement capability indexing as shared source for routing and models listing
- [x] 3.1 Build startup capability snapshots from connector enumeration with config-hint fallback
...
- [x] 3.2 Maintain backend-agnostic canonical model keys with fast candidate lookups
...
- [x] 3.3 Support safe runtime refresh of capability snapshots
  - Replace index snapshots atomically and avoid blocking request processing.
  - _Requirements: 5.4, 7.1, 7.2_

- [x] 3.4 Implement capability refresh control-plane lifecycle policy
...
- [x] 4. Integrate availability and resilience state into candidate eligibility
- [x] 4.1 Filter candidates using permanent disablement and cooldown state
  - Apply instance-level and (instance, model)-level availability checks before selection.
  - _Requirements: 4.1, 4.2, 4.3, 4.5_

- [x] 4.2 Track permanent unsupported-model outcomes and use them in future selection
...
- [x] 4.3 Clear temporary cooldown state on successful recovery attempts
...
- [x] 4.4 Implement explicit backend reactivation control-plane contract
...
- [x] 5. Wire routing behavior into BackendCompletionFlow failover lifecycle
- [x] 5.1 Keep failover attempts bounded and model-aware under the new routing rules
...
- [x] 5.2 Harmonize failure classification across routing, availability, and execution layers
...
- [x] 5.5 Standardize canonical routing error envelope and protocol adapter mappings
  ...
  - _Note: Canonical routing envelope is now mapped across OpenAI-compatible, Anthropic-compatible, and Gemini-compatible error shapes while preserving `details.code` and `retryable` semantics._

- [x] 5.3 Define and enforce proxy-vs-connector precedence rules for routing and wait behavior
...
- [x] 5.4 Define constrained connector-family policy for single-instance self-managed OAuth backends
...
- [x] 6. Integrate B2BUA session semantics with routing and execution identity handling
- [x] 6.1 Preserve A-leg continuity for routing/session context in B2BUA mode
  - Ensure session lookup and routing continuity use canonical A-leg identity rather than transient client-provided ids.
  - _Requirements: 9.1_

- [x] 6.2 Allocate B-leg identity per backend attempt and isolate connector-facing session IDs
...
- [x] 6.3 Isolate auxiliary/sidecar request session lifecycle from primary conversation continuity
...
- [x] 6.4 Route Random Model Replacement backend calls through the shared routing entry point
...
- [x] 6.5 Route Quality Verifier backend calls through the shared routing entry point
...
- [x] 6.6 Enforce no-bypass integration rules for outbound inference routing
...
- [x] 6.7 Preserve connector-internal autonomy for multi-identity connectors
...
- [x] 6.8 Ensure B2BUA/session safety during connector-internal hold and resume windows
...
- [x] 6.9 Enforce single-instance validation for constrained connector families at startup
...
- [x] 6.10 Ensure routing behavior respects constrained connector-family single-instance policy
...
- [x] 7. Update observability surfaces to reflect canonical routing state
- [x] 7.1 Serve canonical backend-agnostic model identifiers from capability index
  - Expose canonical model set for `/v1/models` with compatibility options for legacy clients.
  - _Requirements: 5.3, 6.1_

- [x] 7.2 Extend diagnostics with routing eligibility state and safe correlation metadata
...
- [x] 8. Add verification coverage for routing, availability, capability indexing, and B2BUA behavior
- [x] 8.1 Add unit tests for parser and resolver behavior across all model-addressing variants
...
- [x] 8.2 Add unit tests for capability indexing and runtime refresh behavior
...
- [x] 8.3 Add unit tests for availability filtering and resilience state transitions
...
- [x] 8.4 Add integration tests for request pipeline and completion flow with B2BUA identity handling
...
- [x] 8.5 Add integration tests for API observability and routing errors
...
- [x] 8.6 Run focused tests and quality checks for modified routing and session paths
  - _Requirements: 7.1, 7.2_

- [x] 8.7 Add unification regression tests covering all outbound call categories
...
- [x] 8.8 Add connector-autonomy integration tests for hierarchical routing composition
...
- [x] 8.9 Add configuration validation and migration tests for constrained connector families
...
- [x] 8.10 Add cross-protocol routing-consistency tests
...
- [x] 8.11 Add precedence/override tests for URI parameter inheritance
...
- [x] 8.12 Add reactivation lifecycle tests for permanently disabled instances
...
- [x] 8.13 Add preference-policy and tie-break regression tests for model-only routing
