...
- There is no existing first-class identity scope in the request context contract suitable for scoping session continuity; session resume behavior requires a defined `auth_scope_id` (derived from bearer token identity in multi-user mode, or a fixed implicit scope in single-user localhost mode).
...
| `auth_scope_id` | An identifier representing the caller’s authentication scope used for session continuity mapping: derived from validated bearer token identity in multi-user mode, or a single implicit scope in single-user localhost mode (the raw token value is never used as an identifier). |
