...
- [x] 1. Audit split-path responsibilities and rollout prerequisites before convergence work begins
- [x] 1.1 Characterize the current manager and handler responsibilities at the migration cut line
...
- [x] 1.2 Audit response-processing features and classify their canonical migration strategy
...
- [x] 1.3 Add migration gate configuration, safe defaults, and path-selection diagnostics
...
- [x] 2. Establish the canonical manager-level response-handling contract
- [x] 2.1 Introduce a canonical internal response handle that preserves both the chunk stream and envelope metadata
...
- [x] 2.2 Implement a canonical coordinator that converges post-backend-response business logic at the current manager boundary
...
- [x] 2.3 Implement compatibility adaptation so requested-mode selection happens only at the boundary behind migration gates
...
- [x] 3. Preserve external contract compatibility at API boundaries
- [x] 3.1 Keep streaming transport behavior contract-compatible during canonical-path adoption
...
- [x] 3.2 Keep non-streaming response schema and usage behavior contract-compatible
...
- [x] 3.3 Preserve mode-appropriate error contracts and status extraction behavior
...
- [x] 4. Converge feature-processing contracts to parity-by-construction
- [x] 4.1 Introduce a typed canonical feature context and compatibility bridge for audit-approved features
...
- [x] 4.2 Migrate priority response-processing features and explicitly handle terminal-sensitive or full-response-sensitive cases
...
- [x] 4.3 Replace split-path parity scaffolding with canonical equivalence verification only after feature coverage is complete
...
- [x] 5. Preserve reliability and safety guarantees on the canonical path
- [x] 5.1 Preserve deduplication, cancellation, completion-state tracking, and stream cleanup invariants
...
- [x] 5.2 Preserve empty-response recovery, loop detection, and tool-call retry coordination
...
- [x] 5.3 Preserve quality-verifier, policy, usage, and metadata accounting behavior across requested modes
...
- [x] 6. Migrate connector behavior to stream-first canonical integration
- [x] 6.1 Define a provider capability matrix and connector entry contract around the canonical handle
...
- [x] 6.2 Migrate a first low-risk provider cohort to stream-first connector processing
...
- [x] 6.3 Migrate the remaining providers with explicit adaptation where native streaming is unavailable
...
- [x] 6.4 Remove duplicated connector-side business logic only after provider cohorts meet promotion criteria
...
- [x] 7. Build migration validation evidence and guardrail test suites
- [x] 7.1 Add unit-level coverage for the canonical handle, coordinator, compatibility adapter, and typed feature context
...
- [x] 7.2 Add integration-level success-path and error-path equivalence tests
...
- [x] 7.3 Add safeguard regression coverage for retries, disconnect cleanup, loop detection, and resource release
...
- [x] 7.4 Add performance and promotion-blocking guardrail checks for latency, time-to-first-meaningful-output, memory, and cleanup correctness
...
- [x] 8. Complete staged retirement and final verification
- [x] 8.1 Keep gates default-off, expose stage diagnostics, and codify promotion and rollback criteria
...
- [x] 8.2 Retire legacy handler contracts and parity scaffolding only after all migrated stages pass
...
- [x] 8.3 Run final regression validation and end-to-end operational verification
