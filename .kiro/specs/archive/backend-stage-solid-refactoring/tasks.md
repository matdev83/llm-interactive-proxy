...
- [x] 1. Validation-time DI lifecycle foundations
- [x] 1.1 Add a validation-only provider build capability
...
- [x] 1.2 Add a safe “temporary provider” validation context
...
- [x] 2. Runtime static-route configuration validation
- [x] 2.1 Implement runtime validation for `static_route` on the final resolved configuration
...
- [x] 2.2 Add unit tests for runtime `static_route` validation
...
- [x] 3. Contracts and DI wiring for extracted validation components
- [x] 3.1 Define DI boundary interfaces for validation, HTTP client lifecycle, and init strategies
...
- [x] 3.2 Register extracted validation services via a focused backend registrar
...
- [x] 4. Validation HTTP client manager
- [x] 4.1 Write unit tests for validation HTTP client lifecycle behavior
...
- [x] 4.2 Implement the validation HTTP client manager and integrate it with DI
...
- [x] 5. Backend validation service
- [x] 5.1 Write unit tests for backend validation outcomes and environment behavior
...
- [x] 5.2 Implement backend validation using the canonical backend initialization path
...
- [x] 6. Initialization strategy extraction (Strategy Pattern)
- [x] 6.1 Implement a strategy registry with a default strategy and warning behavior
...
- [x] 6.2 (P) Implement the Anthropic initialization strategy
...
- [x] 6.3 (P) Implement the Gemini initialization strategy
  - Preserve existing backend-specific initialization shaping behavior.
  - Ensure behavior matches current connector expectations without modifying connector runtime behavior.
  - _Requirements: 1.8, 6.5, 14.1_

- [x] 6.4 (P) Implement the OpenRouter initialization strategy
  - Preserve existing backend-specific initialization shaping behavior.
  - Ensure behavior matches current connector expectations without modifying connector runtime behavior.
  - _Requirements: 1.8, 6.5, 14.1_

- [x] 7. Backend factory refactor to eliminate duplication
- [x] 7.1 Refactor backend initialization to delegate augmentation to strategies
...
- [x] 7.2 Add regression-focused tests for strategy-based initialization equivalence
...
- [x] 8. Builder-managed validation lifecycle and config injection
- [x] 8.1 Ensure the builder validates config and prepares DI before stage validation
...
- [x] 8.2 Implement leak-safe stage validation using the validation provider context
...
- [x] 9. BackendStage simplification (delegation-only)
- [x] 9.1 Refactor BackendStage to orchestration + delegation only
...
- [x] 9.2 Update stage validation tests to delegation-only
...
- [x] 10. Test migration and leak-regression repointing
- [x] 10.1 Move static-route validation tests to config-level tests
  - Update tests to validate runtime `static_route` behavior and error payloads.
  - _Requirements: 4.1, 4.2, 4.3, 4.5, 7.7, 12.4_

- [x] 10.2 Add comprehensive unit tests for the backend validation service
...
- [x] 10.3 Add comprehensive unit tests for the validation HTTP client manager
...
- [x] 10.4 Update leak regression tests to target the extracted lifecycle components
...
- [x] 11. Executable “add strategy” example
- [x] 11.1 Add an example strategy and unit test proving OCP-compliant extension
...
- [x] 12. Performance and full verification
- [x] 12.1 Add a lightweight benchmark harness for startup/validation/strategy overhead
...
- [x] 12.2 Run the full test suite and address regressions
