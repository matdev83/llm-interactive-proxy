...
- [x] 1. Extend End-of-Session model for client termination
- [x] 1.1 Add a distinct End-of-Session signal type for client termination
...
- [x] 1.2 Introduce a transport-agnostic session identity used for cancellation and EoS scoping
...
- [x] 2. Ensure session metrics exist and EoS can fail-open
- [x] 2.1 Implement best-effort session metrics initialization with strict timeout
...
- [x] 2.2 Make End-of-Session emission fail-open when persistence is unavailable
...
- [x] 2.3 Add focused unit tests for metrics initialization and fail-open EoS behavior
...
- [x] 3. Implement session-scoped cancellation coordination
- [x] 3.1 Build a session cancellation coordinator with per-session state and bounded retention
...
- [x] 3.2 Add a cancellation gate to prevent any new backend/agentic work after client termination
...
- [x] 3.3 Clean up cancellation state on End-of-Session emission
...
- [x] 4. Normalize client termination signals and orchestrate EoS closure
- [x] 4.1 Implement standardized mapping for legacy and transport-specific cancellation markers
...
- [x] 4.2 Implement a client end-of-session service with idempotent termination reporting
...
- [x] 4.3 Add unit tests for normalization, dedupe, and termination orchestration
...
- [x] 5. Integrate transport-level detection (HTTP + Codebuff)
- [x] 5.1 (P) Report client disconnect during HTTP streaming in a shielded termination hook
...
- [x] 5.2 (P) Report client disconnect/cancellation for HTTP non-streaming requests consistently
...
- [x] 5.3 (P) Report Codebuff WebSocket disconnect as client termination and initialize session metrics
...
- [x] 5.4 Add integration tests for client termination detection across transports
...
- [x] 6. Cancel backend and agentic work when client termination occurs
- [x] 6.1 Update BaseBackendConnector and all implementations to accept explicit cancellation_token
...
- [x] 6.2 Enforce cancellation gating before any backend call, retry, failover, or recovery action
...
- [x] 6.3 Cancel in-flight backend work and scheduled workflow steps for the terminated lifecycle session
...
- [x] 6.4 Treat uncancellable outcomes as non-deliverable after client termination
...
- [x] 6.5 Add integration tests for cancellation scope and recovery suppression
...
- [x] 7. Finalize subsystems and ensure client termination observability
- [x] 7.1 Ensure usage tracking and wire capture finalize with client termination reason on End-of-Session
...
- [x] 7.2 Include client termination reason in ProxyMem finalization when enabled
...
- [x] 7.3 Add observability outputs for client termination and cancellation effects
...
- [x] 7.4 Verify subsystem finalization is robust under partial/early termination
