...
- [x] 1. Establish enforceable quality gates for the DI refactor scope
- [x] 1.1 (P) Extend the existing complexity validator to cover the DI refactor scope
...
- [x] 1.2 (P) Add project-level complexity lint configuration to match the required thresholds
...
- [x] 2. Introduce a registrar orchestration layer with deterministic ordering
- [x] 2.1 Define feature-scoped DI registrars and a single orchestrator entry point
...
- [x] 2.2 Create shared, idempotent registration utilities to reduce duplication
...
- [x] 2.3 Add fast feedback tests for registrar determinism and idempotency
...
- [x] 3. Implement actionable DI resolution diagnostics (resolution path)
- [x] 3.1 (P) Add concurrency-safe resolution path capture when diagnostics are enabled
  ...
  - _Requirements: 1.3_
- [x] 3.2 Add tests for deterministic error shaping across failure modes
...
- [x] 4. Remove DI technical debt by eliminating provider self-healing
- [x] 4.1 Split global provider lifecycle and post-build hooks from registration responsibilities
...
- [x] 4.2 Remove provider rebuild/self-healing behavior and enforce deterministic failure
...
- [x] 4.3 Add regression coverage for removed self-healing behavior
...
- [x] 5. Extract core registration responsibilities into the core registrar
- [x] 5.1 Move foundational registrations into the core registrar without behavior changes
...
- [x] 5.2 Move request processing orchestration registrations into cohesive core modules
...
- [x] 5.3 Validate staged initialization and key service resolutions after core extraction
...
- [x] 6. Extract streaming and response pipeline registrations into the streaming registrar
- [x] 6.1 Move streaming pipeline wiring into a dedicated streaming registrar
...
- [x] 6.2 Ensure post-build feature parity initialization remains correct and isolated
...
- [x] 6.3 Run targeted streaming and DI integrity tests to catch regressions early
  - Run the existing DI integrity and streaming-related tests and fix any wiring issues.
  - _Requirements: 1.4_

- [x] 7. Extract persistence and memory subsystem registrations into cohesive registrars
- [x] 7.1 Move database configuration, engine, and repository registrations into persistence scope
  - Preserve lifetime semantics and interface bindings for repositories and DB-related services.
  - _Requirements: 1.1, 1.2, 2.1_
- [x] 7.2 Move memory subsystem DI wiring with strict "no import-time side effects" discipline
...
- [x] 7.3 Validate that registration does not open connections or perform I/O at import time
  - Add focused checks/tests ensuring persistence wiring remains lazy (connections created on use, not on registration/import).
  - _Requirements: 2.3_

- [x] 8. Extract security, tooling, backend, and resilience wiring; then complete end-to-end validation
- [x] 8.1 Extract sandboxing and security-related registrations into a security registrar
  - Preserve safety feature wiring and interface-first bindings.
  - _Requirements: 2.1, 3.2_
- [x] 8.2 Extract tool-related registrations into a tooling registrar while preserving optional gating
  - Preserve tool access control and tool-call-related wiring behavior.
  - Ensure disabled tool features do not block startup.
  - _Requirements: 2.1, 3.3_
- [x] 8.3 Extract backend and resilience wiring while preserving stage behavior
...
- [x] 8.4 Remove private-helper coupling from stages and align legacy registration paths
...
- [x] 8.5 Run full verification and enforce DI scope quality gates
