...
2.3 When core orchestration invokes a connector backend, the LLM Proxy shall pass the canonical request contract and a minimal connector-facing context contract (e.g., request/session correlation identifiers and JSON-safe extensions) via the canonical connector API without converting them into `dict[str, Any]` payloads; legacy connector invocation may omit the context contract for compatibility, but shall still pass the canonical request contract (see 4.4).
