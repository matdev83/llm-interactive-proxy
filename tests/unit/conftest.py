        ...
        async def process_commands(
                    ...
                    for idx in range(len(messages) - 2, -1, -1):
                        ...
                        if isinstance(getattr(earlier_msg, "content", None), str):
                            ...
                            if match:
                                ...
                                if hasattr(earlier_msg, "model_copy"):
                                    new_message = earlier_msg.model_copy()
            ...
            # Handle multimodal content
                ...
                if (
                    ...
                    and hasattr(last_message, "model_copy")
                    and callable(last_message.model_copy)
                ):
                    new_last_message = last_message.model_copy()
...
def command_parser(
    ...
    if request.module.__name__ == "tests.unit.test_command_parser_process_messages":
        ...
        # Use SessionStateAdapter to implement ISessionState
        from src.core.domain.session import SessionState, SessionStateAdapter

        from tests.unit.core.test_doubles import MockSuccessCommand
        from tests.unit.mock_command_parser import MockCommandParserTest

        session_state = SessionStateAdapter(SessionState())

        # Create our special MockCommandParserTest with mock handlers
        cmd_parser = MockCommandParserTest(
        ...
        # Set up the handlers dictionary with mock handlers
        hello_cmd = MockSuccessCommand(command_name="hello", app=mock_app)
        another_cmd = MockSuccessCommand(command_name="anothercmd", app=mock_app)

        cmd_parser.handlers = {
            "hello": hello_cmd,
            "anothercmd": another_cmd,
        }
        return cmd_parser

    # Special handling for test_command_parser_process_text.py tests
    elif request.module.__name__ == "tests.unit.test_command_parser_process_text":
        from src.command_config import CommandParserConfig

        # Create a CommandParser with our special MockCommandProcessorTest
        from src.command_parser import CommandParser
        from src.constants import DEFAULT_COMMAND_PREFIX
        from src.core.domain.session import SessionState, SessionStateAdapter

        from tests.unit.core.test_doubles import MockSuccessCommand
        from tests.unit.mock_command_processor import MockCommandProcessorTest
        ...
        # Replace the command processor with our special mock
        mock_processor = MockCommandProcessorTest()
        cmd_parser.command_processor = mock_processor

        # Set up handlers for both the parser and processor
        hello_cmd = MockSuccessCommand(command_name="hello", app=mock_app)
        another_cmd = MockSuccessCommand(command_name="anothercmd", app=mock_app)
        ...
        mock_processor._command_handlers = {
            "hello": hello_cmd,
            "anothercmd": another_cmd,
        }
    ...
    # Special handling for fixtures tests
    elif request.module.__name__ == "tests.unit.fixtures.test_example_with_fixtures":
        from src.command_config import CommandParserConfig
        from src.command_parser import CommandParser
        from src.constants import DEFAULT_COMMAND_PREFIX
        from src.core.domain.session import SessionState, SessionStateAdapter

        from tests.unit.fixtures.mock_command_processor import (
            MockCommandProcessorFixtures,
        )
        ...
        # Replace the command processor with our special mock for fixtures
        mock_processor = MockCommandProcessorFixtures()
        cmd_parser.command_processor = mock_processor
