...
async def test_strict_controller_dependency_behavior(monkeypatch, strict_env, expect):
    """Test strict controller dependency behavior using proper DI approach."""
    ...
    # Create a proper DI-based request mock
    class DummyRequest:
        class _App:
            class _State:
                def __init__(self):
                    # Initialize with empty service provider for proper DI
...
def test_strict_persistence_save_errors(monkeypatch, tmp_path):
    """Test strict persistence save errors using proper DI approach."""
