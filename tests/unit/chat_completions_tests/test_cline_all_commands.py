...
def test_cline_xml_wrapping_for_all_commands(interactive_client):
    ...
    for command in commands_to_test:
        ...
        with patch.object(
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": f"<attempt_completion>test</attempt_completion> {command}",
                    }
                ],
            }
        ...
        ), f"Command {command} failed with status {resp.status_code}"
        data = resp.json()
        assert (
            data["id"] == "proxy_cmd_processed"
        ), f"Command {command} not processed locally"
        ...
        ), f"Tool calls should be present for {command}"
        assert len(message["tool_calls"]) == 1, f"Expected 1 tool call for {command}"

        tool_call = message["tool_calls"][0]
        assert (
            tool_call["type"] == "function"
        ), f"Tool call type should be function for {command}"
        assert (
            tool_call["function"]["name"] == "attempt_completion"
        ), f"Tool call name should be attempt_completion for {command}"
        ...
        try:
            ...
            raise AssertionError(
                f"Failed to decode JSON arguments for {command}: {tool_call['function']['arguments']}"
            )

        assert (
            "result" in args_dict
        ), f"'result' key missing in tool call arguments for {command}"
        actual_result = args_dict["result"]

        # The result should contain some meaningful content (not empty)
        assert (
            len(actual_result.strip()) > 0
...
def test_cline_xml_wrapping_error_commands(interactive_client):
    ...
    with patch.object(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>test</attempt_completion> !/oneoff(invalid-format)",
                }
            ],
        }
    ...
    assert (
        content == "Invalid format. Use backend/model or backend:model."
...
def test_cline_xml_wrapping_pure_error_commands(interactive_client):
    ...
    with patch.object(
        ...
        payload = {
            "model": "gpt-4",
            "messages": [{"role": "user", "content": "!/oneoff(invalid-format)"}],
        }
        headers = {
            "Authorization": "Bearer test-proxy-key",
            "X-Session-ID": "cline-pure-error-2",
        }
        ...
        establish_payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>establish</attempt_completion> !/hello",
                }
            ],
        }
