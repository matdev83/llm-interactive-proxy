...
async def test_non_cline_clients_no_xml_wrapping(interactive_client):
    ...
    for command in commands_to_test:
        ...
        assert (
            not session.state.is_cline_agent
...
async def test_remote_llm_responses_never_xml_wrapped(interactive_client):
    ...
    with patch.object(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>Please help me with Python</attempt_completion> What is a list comprehension?",
                }
            ],
        }
    ...
    assert session.state.is_cline_agent, "Cline agent should be detected"
...
async def test_mixed_cline_command_and_llm_prompt(interactive_client):
    ...
    with patch.object(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>setup</attempt_completion> !/set(backend=openrouter)",
                }
            ],
        }
    ...
    assert not llm_content.startswith(
        "<attempt_completion>"
    ), "LLM response should NOT be XML wrapped"
    assert (
        llm_content == "Here's how to use Python lists effectively."
...
async def test_streaming_responses_never_xml_wrapped(interactive_client):
    ...
    with patch.object(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>stream test</attempt_completion> Tell me about Python",
                }
            ],
            "stream": True,
        }
        ...
        headers = {
            "Authorization": "Bearer test-proxy-key",
            "X-Session-ID": "cline-stream-test",
        }
        resp = interactive_client.post(
            "/v1/chat/completions", json=payload, headers=headers
        )
    ...
    assert session.state.is_cline_agent, "Cline agent should be detected for streaming"
    ...
    with patch.object(
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "<attempt_completion>error test</attempt_completion> This will cause an error",
                }
            ],
        }
