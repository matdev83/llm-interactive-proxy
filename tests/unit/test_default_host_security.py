...
try:
    ...
    print("Error: Cannot import AppConfig. This test should be run with pytest or with proper Python path setup.")
...
def test_default_host_is_localhost():
    ...
    with patch.dict(os.environ, {}, clear=True):
        config = AppConfig()
        assert config.host == "127.0.0.1", f"Expected default host to be '127.0.0.1', but got '{config.host}'"
        assert config.host != "0.0.0.0", "Security regression: default host should not be '0.0.0.0'"
...
def test_default_host_from_env_still_uses_localhost():
    ...
    with patch.dict(os.environ, env_copy, clear=True):
        ...
        assert config.host == "127.0.0.1", f"Expected default host to be '127.0.0.1' from env, but got '{config.host}'"
...
def test_host_can_be_overridden():
    ...
    with patch.dict(os.environ, {'APP_HOST': '0.0.0.0'}):
        config = AppConfig.from_env()
        assert config.host == "0.0.0.0", f"Expected host to be '0.0.0.0' when explicitly set, but got '{config.host}'"
...
def test_default_config_host_field():
    ...
    assert config.host not in unsafe_defaults, f"Host should not default to any of {unsafe_defaults}"
