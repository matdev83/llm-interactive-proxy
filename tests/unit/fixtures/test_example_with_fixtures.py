...
@pytest.mark.backend
@pytest.mark.asyncio
async def test_backend_service_fixture(
    backend_service, mock_backend, mock_backend_factory
):
...
async def test_command_parser_fixture(command_parser, test_session):
    ...
    # Process the message
    processed_messages, commands_found = await command_parser.process_messages(
        [message]
    )
...
def test_multimodal_message_fixture(multimodal_message):
    ...
    assert (
        multimodal_message.content[1].image_url.url == "https://example.com/image.jpg"
    )
...
async def test_multimodal_message_with_command_fixture(
    multimodal_message_with_command, process_command
):
    ...
    processed_messages, commands_found, processed_text = await process_command(
        text_part.text
    )
