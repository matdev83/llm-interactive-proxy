...
class TestRequestRedactionMiddleware:
    ...
    async def test_process_request_with_message_parts(self):
        ...
        assert (
            ...
            == "test (API_KEY_HAS_BEEN_REDACTED) message"
        )
...
class TestResponseRedactionMiddleware:
    ...
    async def test_process_non_streaming_response_with_redaction(self):
                        ...
                        if "message" in choice and "content" in choice["message"]:
                            ...
                            if content:
                                choice["message"]["content"] = (
                                    self.api_key_redactor.redact(content)
                                )
                return response
        ...
        response = {
            "choices": [
                {"message": {"content": "This is a response with SECRET_RESPONSE key"}}
            ]
        }

        result: dict[str, Any] = cast(
            dict[str, Any], await middleware.process_response(response, context)
        )
        # Should redact API key in response
        assert (
            result["choices"][0]["message"]["content"]
            == "This is a response with (API_KEY_HAS_BEEN_REDACTED) key"
        )
...
class TestGlobalMiddlewareConfiguration:
    ...
    def test_configure_redaction_middleware(self):
        ...
        redaction_processors = [
            p for p in middleware.middleware_stack if isinstance(p, RedactionProcessor)
        ]
