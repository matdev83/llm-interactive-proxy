...
class TestQwenOAuthConnectorUnit:
    ...
    def test_connector_initialization(self, connector, mock_client):
        ...
        assert connector._default_endpoint == "https://dashscope.aliyuncs.com/compatible-mode/v1"
    ...
    def test_get_headers_no_token(self, connector):
        ...
        assert "No valid Qwen OAuth access token available" in str(exc_info.value.detail)
    ...
    async def test_chat_completions_success(self, connector, mock_client):
        ...
        with patch.object(connector, '_refresh_token_if_needed', AsyncMock(return_value=True)), \
             patch.object(connector, '_prepare_payload', return_value={"model": "qwen3-coder-plus", "messages": [{"role": "user", "content": "Hello"}]}), \
             patch.object(connector, '_handle_non_streaming_response', AsyncMock(return_value=(mock_response.json.return_value, mock_response.headers))):
    ...
    async def test_chat_completions_with_prefix(self, connector, mock_client):
        ...
        parent_mock = AsyncMock(return_value=(mock_response.json.return_value, mock_response.headers))
        
        # Mock parent class methods
        with patch.object(connector, '_refresh_token_if_needed', AsyncMock(return_value=True)), \
             patch('src.connectors.openai.OpenAIConnector.chat_completions', parent_mock):
    ...
    async def test_chat_completions_streaming(self, connector, mock_client):
        ...
        # Mock parent class methods
        with patch.object(connector, '_refresh_token_if_needed', AsyncMock(return_value=True)), \
             patch('src.connectors.openai.OpenAIConnector.chat_completions', AsyncMock(return_value=mock_stream_response)):
    ...
    async def test_chat_completions_token_refresh_failure(self, connector):
        ...
        with patch.object(connector, '_refresh_token_if_needed', AsyncMock(return_value=False)):
            with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_chat_completions_exception_handling(self, connector, mock_client):
        ...
        with patch.object(connector, '_refresh_token_if_needed', AsyncMock(return_value=True)), \
             patch('src.connectors.openai.OpenAIConnector.chat_completions', AsyncMock(side_effect=Exception("Test error"))):
