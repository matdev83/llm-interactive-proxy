...
class TestProcessTextForCommands:
    ...
    def test_set_and_unset_project(self):
        ...
        processed_text, _ = _process_text_for_commands(
            "!/unset(project)", current_proxy_state, pattern, app=self.mock_app
        )
        assert processed_text == ""
    ...
    def test_set_interactive_mode(self):
        ...
        processed_text, found = _process_text_for_commands(
            text, current_proxy_state, pattern, app=self.mock_app
        )
    ...
    def test_set_backend(self):
        ...
        processed, found = _process_text_for_commands(
            ...
            state,
            pattern,
            functional_backends=functional_backends_for_test,
            app=self.mock_app,  # Pass it
        )
        assert found
        assert processed == "hi"
    ...
    def test_unset_backend(self):
        ...
        text = "!/unset(backend)"
        processed, found = _process_text_for_commands(
            text, state, pattern, app=self.mock_app
        )
        assert found
        assert processed == ""
        assert state.override_backend is None
