...
def test_specific_validation_constants():
    ...
    assert (
        MODEL_BACKEND_NOT_SUPPORTED_MESSAGE
        == "Backend {backend} in model {model} not supported"
    )
    ...
    assert (
        TOOL_LOOP_MAX_REPEATS_MUST_BE_AT_LEAST_TWO_MESSAGE
        == "Max repeats must be at least 2"
    )
    assert (
...
def test_validation_constant_formatting():
    ...
    formatted_integer = TOOL_LOOP_MAX_REPEATS_MUST_BE_INTEGER_MESSAGE.format(
        value="abc"
    )
    ...
    formatted_at_least = VALIDATION_MUST_BE_AT_LEAST_MESSAGE.format(
        field="repeats", min_value=2
    )
    assert formatted_at_least == "repeats must be at least 2"

    formatted_between = VALIDATION_MUST_BE_BETWEEN_MESSAGE.format(
        field="temperature", min_value=0.0, max_value=1.0
    )
    ...
    formatted_format = VALIDATION_INVALID_FORMAT_MESSAGE.format(
        field="url", value="invalid_url"
    )
    assert formatted_format == "Invalid url format: invalid_url"

    formatted_not_supported = VALIDATION_NOT_SUPPORTED_MESSAGE.format(
        field="backend", value="invalid_backend"
    )
    assert formatted_not_supported == "backend invalid_backend not supported"

    # Test specific validation message formatting
    formatted_backend_not_supported = BACKEND_NOT_SUPPORTED_MESSAGE.format(
        backend="invalid_backend"
    )
    assert formatted_backend_not_supported == "Backend invalid_backend not supported"

    formatted_model_backend_not_supported = MODEL_BACKEND_NOT_SUPPORTED_MESSAGE.format(
        backend="invalid_backend", model="test:model"
    )
    assert (
        formatted_model_backend_not_supported
        == "Backend invalid_backend in model test:model not supported"
    )

    formatted_temperature_between = TEMPERATURE_MUST_BE_BETWEEN_MESSAGE.format(
        min_temp=0.0, max_temp=2.0
    )
    assert formatted_temperature_between == "Temperature must be between 0.0 and 2.0"

    formatted_max_repeats_integer = (
        TOOL_LOOP_MAX_REPEATS_MUST_BE_INTEGER_MESSAGE.format(value="not_a_number")
    )
    assert (
        formatted_max_repeats_integer
        == "Invalid max repeats value: not_a_number. Must be an integer."
    )

    formatted_ttl_integer = TOOL_LOOP_TTL_MUST_BE_INTEGER_MESSAGE.format(
        value="not_a_number"
    )
    assert (
        formatted_ttl_integer == "Invalid TTL value: not_a_number. Must be an integer."
    )

    formatted_mode_invalid = TOOL_LOOP_MODE_INVALID_MESSAGE.format(value="invalid_mode")
    assert (
        formatted_mode_invalid
        == "Invalid loop mode: invalid_mode. Use break or chance_then_break."
    )
