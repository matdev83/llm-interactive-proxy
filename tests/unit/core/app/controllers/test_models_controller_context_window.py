...
async def test_list_models_populates_context_window():
    ...
    with patch(
        "src.core.app.controllers.models_controller.backend_registry"
    ) as mock_registry:
        ...
        gemini_backend.get_available_models_async = AsyncMock(
            return_value=["gemini-1.5-pro", "gemini-3-flash-preview"]
        )
        ...
        if "google:gemini-3-flash-preview" in model_ids:
            gemini3 = next(
                m for m in response.data if m.id == "google:gemini-3-flash-preview"
            )
