...
class TestFilterNewCalls:
    ...
    def test_filter_new_calls_with_buffered_calls(self) -> None:
    ...
    def test_filter_new_calls_with_non_buffered_calls(self) -> None:
    ...
    def test_filter_new_calls_skips_already_processed(self) -> None:
    ...
    def test_filter_new_calls_skips_duplicate_detections(self) -> None:
        ...
        result1 = resolver.filter_new_calls(
            [call1], stream_key, None, is_streaming=False
        )
        assert len(result1) == 1

        # Second detection should be skipped
        result2 = resolver.filter_new_calls(
            [call1], stream_key, None, is_streaming=False
        )
        assert len(result2) == 0

    def test_filter_new_calls_handles_none_buffer_state(self) -> None:
    ...
    def test_filter_new_calls_empty_list(self) -> None:
        ...
        result = resolver.filter_new_calls([], stream_key, None, is_streaming=False)

        assert result == []

    def test_filter_new_calls_mixed_buffered_and_non_buffered(self) -> None:
        ...
        buffered_result = resolver.filter_new_calls(
            [], stream_key, buffer_state, is_streaming=True
        )
        non_buffered_result = resolver.filter_new_calls(
...
class TestMarkProcessed:
    ...
    def test_mark_processed_updates_lifecycle_registry(self) -> None:
        ...
        # Mark as processed
        resolver.mark_processed(stream_key, signature, None)

        # Verify it's marked as processed
        assert resolver.is_processed(stream_key, signature)

    def test_mark_processed_updates_buffer_state(self) -> None:
        ...
        # Mark as processed
        resolver.mark_processed(stream_key, signature, buffer_state)

        # Verify it's in buffer processed signatures
        assert signature in buffer_state_obj.processed_signatures

    def test_mark_processed_handles_none_buffer_state(self) -> None:
...
class TestIsProcessed:
    ...
    def test_is_processed_returns_false_for_new_signature(self) -> None:
        ...
        assert not resolver.is_processed(stream_key, signature)

    def test_is_processed_returns_true_after_marking(self) -> None:
    ...
    def test_is_processed_handles_different_streams(self) -> None:
        ...
        # Mark in stream 1
        resolver.mark_processed("stream_1", signature, None)
        assert resolver.is_processed("stream_1", signature)

        # Should not be processed in stream 2
        assert not resolver.is_processed("stream_2", signature)
