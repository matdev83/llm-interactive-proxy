...
class TestPlanningPhaseManagerEquivalence:
    """Equivalence tests comparing PlanningPhaseManager to BackendService methods."""

    @pytest.mark.asyncio
    async def test_apply_if_needed_equivalence(self) -> None:
        """PlanningPhaseManager.apply_if_needed should match BackendService behavior."""
        ...
        session1 = Session(session_id="test1", state=state1)
        session2 = Session(session_id="test2", state=state2)

        # Apply with both
        await manager.apply_if_needed(session1, "anthropic")
        await backend_service._apply_planning_phase_if_needed(session2, "anthropic")

        # Results should match
        assert (
            session1.state.backend_config.model == session2.state.backend_config.model
        )
        assert (
            session1.state.backend_config.backend_type
            == session2.state.backend_config.backend_type
        )
        assert (
            session1.state.planning_phase_original_backend
            == session2.state.planning_phase_original_backend
        )
        assert (
            session1.state.planning_phase_original_model
            == session2.state.planning_phase_original_model
        )

    def test_count_file_writes_equivalence(self) -> None:
        """PlanningPhaseManager.count_file_writes should match BackendService behavior."""
        from src.core.config.app_config import AppConfig
        from src.core.services.backend_factory import BackendFactory
        from src.core.services.backend_service import BackendService
        ...
        # Test with various tool call combinations
        test_cases: list[dict[str, Any]] = [
            {"tool_calls": []},
            {"tool_calls": [{"function": {"name": "write_file"}, "id": "1"}]},
            {"tool_calls": [{"function": {"name": "read_file"}, "id": "1"}]},
            {
                "tool_calls": [
                    {"function": {"name": "write_file"}, "id": "1"},
                    {"function": {"name": "edit_file"}, "id": "2"},
        ...
        for metadata in test_cases:
            ...
            manager_count = manager.count_file_writes(response)
            bs_count = backend_service._count_file_writes_in_response(response)

            assert manager_count == bs_count, f"Mismatch for {metadata}"
