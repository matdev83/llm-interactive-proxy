...
class DummyAngelFactory:
    def __init__(self, service: AngelService) -> None:
        self._service = service

    def create(self, *args: Any, **kwargs: Any) -> AngelService:
...
async def test_stream_verifier_retries_once_on_invalid_format_then_steers() -> None:
    angel_service = AngelService("openai:gpt-4o-mini")
            ...
            if call_index == 2:
                ...
                return type(
                    ...
                    (),
                    {
                        ...
                        "<angels_steering_message>Fix result</angels_steering_message>"
                    },
                )()
    ...
    verifier = AngelStreamVerifier(
        angel_service_factory=DummyAngelFactory(angel_service),
    ...
    context: dict[str, Any] = {
        "angel_model_spec": "openai:gpt-4o-mini",
        "angel_frequency": 1,
    }
    ...
    assert result_chunks[0].metadata.get("corrected_by_angel") is True
