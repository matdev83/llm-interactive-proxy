...
from src.core.common.exceptions import AuthenticationError, BackendError, InvalidRequestError
...
async def test_auth_failure_permanent_backend_disable(backend_service):
    ...
    backend_type = "openai"
    backend = MockBackend()
    backend.chat_completions = AsyncMock(side_effect=AuthenticationError("Invalid API Key"))
    
    # Mock resolve to return our backend type
    backend_service._resolve_backend_and_model = AsyncMock(return_value=("openai", "gpt-4", {}))
    # Mock backend creation
    backend_service._get_or_create_backend = AsyncMock(return_value=backend)
    
    request = ChatRequest(messages=[ChatMessage(role="user", content="hi")], model="gpt-4")
...
async def test_backend_error_401_permanent_disable(backend_service):
    ...
    backend_type = "openai"
    backend = MockBackend()
    backend.chat_completions = AsyncMock(side_effect=BackendError("Unauthorized", status_code=401))
...
async def test_http_exception_401_permanent_disable(backend_service):
    ...
    backend_type = "openai"
    backend = MockBackend()
    backend.chat_completions = AsyncMock(side_effect=HTTPException(status_code=401, detail="Unauthorized"))
...
async def test_oauth_backend_not_permanently_disabled(backend_service):
    ...
    backend.chat_completions = AsyncMock(side_effect=AuthenticationError("Token expired"))

    backend_service._resolve_backend_and_model = AsyncMock(return_value=("gemini-oauth", "gemini-2.5-pro", {}))
    backend_service._get_or_create_backend = AsyncMock(return_value=backend)

    request = ChatRequest(messages=[ChatMessage(role="user", content="hi")], model="gemini-2.5-pro")
