...
async def test_process_request_with_commands():
    ...
    # Create a request
    request = MockRequest(headers={"x-session-id": "test-session"})
    request_data = MockRequestData(messages=[
        {"role": "user", "content": "!/set(project=test) How are you?"}
    ])
    ...
    command_service.add_result(
        ProcessedResult(
            ...
            command_results=[
                CommandResult(success=True, message="Project set to test", data={"name": "set"})
            ]
        )
    )
    ...
    session_service.sessions["test-session"].state.project = "test"  # This is a mock, so we can set it directly
...
async def test_process_command_only_request():
    ...
    command_service.add_result(
        ProcessedResult(
            ...
            command_executed=True,
            command_results=[
                CommandResult(success=True, message="Hello acknowledged", data={"name": "hello"})
            ]
        )
    )
...
async def test_process_streaming_request():
    ...
    backend_service._backend_service.call_completion = AsyncMock(return_value=mock_stream_response())
    
    with patch.object(backend_service, "call_completion", return_value=mock_stream_response()):
        # Act
        ...
        assert json.loads(chunks[0].replace("data: ", ""))["choices"][0]["delta"]["content"] == "Hello"
        assert json.loads(chunks[1].replace("data: ", ""))["choices"][0]["delta"]["content"] == " there!"
