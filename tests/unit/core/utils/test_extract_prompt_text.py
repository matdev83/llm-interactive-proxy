...
def test_extract_prompt_text_with_reasoning_content():
    ...
    messages_dict = [
        {"role": "assistant", "reasoning_content": "I should say hello.", "content": "Hi!"}
    ]
    assert "assistant (reasoning): I should say hello." in extract_prompt_text(messages_dict)
    assert "assistant: Hi!" in extract_prompt_text(messages_dict)
    
    # Test reasoning_content in object
    messages_obj = [
        ChatMessage(role="assistant", reasoning_content="Logic here", content="Result here")
    ]
