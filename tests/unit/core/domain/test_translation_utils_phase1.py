...
def test_json_utils_sanitize_dict_drops_non_json_values() -> None:
    ...
    assert json_utils.sanitize_dict_for_json(payload) == expected
    assert Translation.sanitize_dict_for_json(payload) == expected
...
def test_json_utils_sanitize_dict_handles_cycles() -> None:
    ...
    assert json_utils.sanitize_dict_for_json(payload) == {"self": {}}
    assert Translation.sanitize_dict_for_json(payload) == {"self": {}}
...
def test_json_utils_sanitize_list_handles_cycles() -> None:
    ...
    assert json_utils.sanitize_list_for_json(payload) == [[]]
    assert Translation.sanitize_list_for_json(payload) == [[]]


def test_tool_utils_normalize_tool_arguments_accepts_valid_json_string() -> None:
    raw = '  {"a": 1}  '
    assert tool_utils.normalize_tool_arguments(raw) == '{"a": 1}'
    assert Translation.normalize_tool_arguments(raw) == '{"a": 1}'


def test_tool_utils_normalize_tool_arguments_fixes_simple_single_quotes() -> None:
    raw = "{'a': 1}"
    normalized = tool_utils.normalize_tool_arguments(raw)
...
def test_tool_utils_normalize_tool_arguments_rejects_unfixable_jsonish_strings() -> (
    ...
    raw = "{'a': \"can't\"}"
    assert tool_utils.normalize_tool_arguments(raw) == "{}"
...
def test_tool_utils_process_gemini_function_call_preserves_thought_signature() -> None:
    ...
    tool_call = tool_utils.process_gemini_function_call(function_call, part=part)
    assert tool_call.id == "call_123"
    assert tool_call.function.name == "do_thing"
    assert json.loads(tool_call.function.arguments or "{}") == {"x": 1}
    assert tool_call.extra_content == {"google": {"thought_signature": "sig_abc"}}


def test_media_utils_detect_image_mime_type_data_uri() -> None:
    assert (
        media_utils.detect_image_mime_type("data:image/png;base64,AAAA") == "image/png"
    )


def test_media_utils_process_gemini_image_part_inline_data() -> None:
    part = MessageContentPartImage(
        image_url=ImageURL(url="data:image/png;base64,AAAA", detail=None)
    )
    assert media_utils.process_gemini_image_part(part) == {
...
def test_media_utils_process_gemini_image_part_rejects_file_scheme_and_local_paths() -> (
    ...
    part_file = MessageContentPartImage(
        image_url=ImageURL(url="file:///etc/passwd", detail=None)
    )
    assert media_utils.process_gemini_image_part(part_file) is None

    part_local = MessageContentPartImage(
        image_url=ImageURL(url="C:\\\\tmp\\\\x.png", detail=None)
    )
    assert media_utils.process_gemini_image_part(part_local) is None
...
def test_content_utils_coerce_reasoning_text_picks_common_keys() -> None:
    ...
    assert content_utils.coerce_reasoning_text(payload) == " one \ntwo"


def test_content_utils_safe_string_handles_bytes_and_none() -> None:
    assert content_utils.safe_string(None) == ""
    assert content_utils.safe_string(b"hi") == "hi"
...
def test_usage_utils_openai_preserves_token_details() -> None:
    ...
    normalized = usage_utils.normalize_usage_metadata(usage, "openai")
