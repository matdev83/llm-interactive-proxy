...
class TestQwenOAuthInteractiveCommands:
    ...
    def test_interactive_model_setting_issue(self):
        ...
        assert "model must be specified as <backend>:<model> or <backend>/<model>" in result.message
    ...
    def test_backend_attribute_name_conversion(self):
        ...
        }), patch('src.connectors.qwen_oauth.QwenOAuthConnector._load_oauth_credentials', return_value=True), \
             patch('src.connectors.qwen_oauth.QwenOAuthConnector.get_available_models', return_value=["qwen3-coder-plus"]):
    ...
    def test_fixed_interactive_model_setting(self):
        ...
        assert result.success is False
        assert "Backend 'qwen-oauth' for model not available/configured" in result.message
...
class TestQwenOAuthConfigurationMethods:
    ...
    def test_config_file_backend_persistence(self):
        ...
        with patch.dict(os.environ, {
            ...
            config_manager = ConfigManager(app, path=":memory:")  # Provide required path parameter
    ...
    def test_all_backend_access_methods(self):
        ...
        with patch.dict(os.environ, {
            ...
            backend_obj_2 = getattr(app.state, f"{BackendType.QWEN_OAUTH.replace('-', '_')}_backend", None)
