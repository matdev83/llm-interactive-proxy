...
class TestKiloCodeToolExecutor:
    ...
    async def test_read_file_not_found(self, executor, temp_workspace):
        ...
        result = await executor.execute_tool("read_file", {"file_path": "nonexistent.txt"})
    ...
    async def test_list_dir_recursive(self, executor, temp_workspace):
        ...
        result = await executor.execute_tool("list_dir", {
            "dir_path": ".",
            "recursive": True
        })
    ...
    async def test_grep_files_success(self, executor, temp_workspace):
        ...
        result = await executor.execute_tool("grep_files", {
            "pattern": "Hello",
            "path": "."
        })
        
        assert result["exit_code"] == 0
        assert "test_file.txt" in result["output"] or "nested_file.py" in result["output"]
    ...
    async def test_completion_marker(self, executor, temp_workspace):
        ...
        result = await executor.execute_tool("completion_marker", {
    ...
    async def test_followup_marker(self, executor, temp_workspace):
        ...
        result = await executor.execute_tool("followup_marker", {
            "question": "Do you need help with anything else?"
        })
    ...
    async def test_tool_execution_exception(self, executor, temp_workspace):
        ...
        with patch.object(executor, '_execute_read_file', side_effect=Exception("Test error")):
...
class TestKiloCodeToolExecutorIntegration:
    ...
    async def test_file_operations_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            read_result = await executor.execute_tool("read_file", {"file_path": "workflow_test.py"})
            ...
            grep_result = await executor.execute_tool("grep_files", {
                "pattern": "def main",
                "path": "."
            })
    ...
    async def test_error_handling_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test reading non-existent file
            result = await executor.execute_tool("read_file", {"file_path": "missing.txt"})
            assert result["exit_code"] == 1
            
            # Test listing non-existent directory
            result = await executor.execute_tool("list_dir", {"dir_path": "missing_dir"})
            assert result["exit_code"] == 1
