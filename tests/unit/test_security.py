...
def test_cli_disable_auth_forces_localhost():
    ...
    with patch.dict(os.environ, {}, clear=True), patch("uvicorn.run") as mock_uvicorn, patch("src.core.cli.logging.basicConfig"), patch("src.core.cli._check_privileges"):
...
def test_env_disable_auth_forces_localhost():
    ...
    with patch.dict(os.environ, {"DISABLE_AUTH": "true", "PROXY_HOST": "0.0.0.0"}, clear=True), patch("uvicorn.run") as mock_uvicorn, patch("src.core.cli.logging.basicConfig"), patch("src.core.cli._check_privileges"):
        from src.core.cli import main
        mock_app = object()
        def mock_build_app(cfg, config_file=None):
            return mock_app
...
def test_auth_enabled_allows_custom_host():
    ...
    with patch.dict(os.environ, {"DISABLE_AUTH": "false", "PROXY_HOST": "0.0.0.0"}, clear=True), patch("uvicorn.run") as mock_uvicorn, patch("src.core.cli.logging.basicConfig"), patch("src.core.cli._check_privileges"):
        from src.core.cli import main
        mock_app = object()
        def mock_build_app(cfg, config_file=None):
            return mock_app
