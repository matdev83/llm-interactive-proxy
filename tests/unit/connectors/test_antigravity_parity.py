...
class TestAntigravityParity:
    ...
    def test_account_block_message_detection(self, connector):
        ...
        assert (
            connector._is_account_blocked_message("Forbidden", status_code=403) is False
        )
    ...
    async def test_chat_completions_handles_account_block(self, connector):
        ...
        with patch(
            ...
            assert (
                getattr(exc_info.value, "__resilience_context__", {}).get(
                    "is_personal_backend"
                )
                is True
            )
    ...
    async def test_chat_completions_logic_amplification_protection(self, connector):
        ...
        with patch(
            ...
            error_any: Any = exc_info.value
            assert error_any.__rate_limit_recorded__ is True
