...
def _archive() -> bytes:
    ...
    with zipfile.ZipFile(output, "w") as package:
        package.writestr("go-freebuff-acp-wrapper.exe", b"freebuff-wrapper")
...
async def test_install_latest_downloads_verifies_and_caches(
    ...
    archive = _archive()
    asset_name = "go-freebuff-acp-wrapper_1.0.0_windows_amd64.zip"
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        "src.connectors.freebuff_acp_wrapper_installer.platform_asset_suffix",
    ...
    assert first == second
    assert Path(first).read_bytes() == b"freebuff-wrapper"
...
async def test_install_latest_rejects_bad_checksum_without_cache(
    ...
    assert not list(tmp_path.glob("*/go-freebuff-acp-wrapper.exe"))


@pytest.mark.asyncio
async def test_install_latest_uses_cached_binary_offline(tmp_path: Path) -> None:
    cached = tmp_path / "v1.0.0" / "go-freebuff-acp-wrapper.exe"
