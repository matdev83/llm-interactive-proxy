...
async def test_anthropic_streaming_translates_to_domain_format():
    ...
    # Mock the client.send to return our mock response
    with patch.object(backend.client, 'build_request', return_value=MagicMock()):
        with patch.object(backend.client, 'send', return_value=mock_response):
            ...
            assert len(chunks) > 0, f"Expected chunks but got none"
            
            # Check that we got domain-formatted chunks
            content_chunks = [c for c in chunks if isinstance(c, dict) and c.get("choices")]
            assert len(content_chunks) > 0, f"Should have domain-formatted chunks. Got: {chunks[:3]}"
            ...
            for i, chunk in enumerate(content_chunks):
                ...
                assert chunk["object"] == "chat.completion.chunk", f"Chunk {i} wrong object type: {chunk['object']}"
                assert "choices" in chunk, f"Chunk {i} missing 'choices': {chunk}"
                assert len(chunk["choices"]) > 0, f"Chunk {i} has empty choices"
                assert "delta" in chunk["choices"][0], f"Chunk {i} missing 'delta': {chunk['choices'][0]}"
                assert "index" in chunk["choices"][0], f"Chunk {i} missing 'index': {chunk['choices'][0]}"
            ...
            assert full_content, f"Expected content but got empty. Chunks: {[c['choices'][0]['delta'] for c in content_chunks[:5]]}"
            assert "Hello" in full_content or "world" in full_content, f"Expected 'Hello' or 'world' in content, got: '{full_content}'"
