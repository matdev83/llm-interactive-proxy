...
async def test_chat_completions_unpacks_canonical_request():
    ...
    # Act & Assert
    # We patch the immediate parent class's chat_completions method.
    # Note: We patch where the class is defined/imported.
    # GeminiOAuthPlanConnector inherits from GeminiOAuthBaseConnector.
    with patch("src.connectors.gemini_oauth_base.GeminiOAuthBaseConnector.chat_completions", new_callable=AsyncMock) as mock_super_chat:
