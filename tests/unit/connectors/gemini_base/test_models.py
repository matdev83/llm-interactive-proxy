...
class TestGeminiOAuthCredentials:
    ...
    def test_access_token_validation(self) -> None:
        ...
        with pytest.raises(ValueError, match="access_token"):
            GeminiOAuthCredentials(access_token=None)  # type: ignore[arg-type]

    def test_optional_fields(self) -> None:
        """Verify that optional fields can be None."""
        creds = GeminiOAuthCredentials(access_token="test_token")
    ...
    def test_refresh_token_validation(self) -> None:
        ...
        with pytest.raises(ValueError, match="refresh_token"):
            GeminiOAuthCredentials(access_token="test", refresh_token="")

        # None is allowed
        creds = GeminiOAuthCredentials(access_token="test", refresh_token=None)
        assert creds.refresh_token is None
    ...
    def test_expiry_date_validation(self) -> None:
        ...
        with pytest.raises(ValueError, match="expiry_date"):
            GeminiOAuthCredentials(access_token="test", expiry_date=-1)

        # None is allowed
        creds = GeminiOAuthCredentials(access_token="test", expiry_date=None)
        assert creds.expiry_date is None
    ...
    def test_is_expired_not_expired(self) -> None:
        ...
        creds = GeminiOAuthCredentials(
            access_token="test", expiry_date=future_timestamp
        )
    ...
    def test_has_refresh_token(self) -> None:
        ...
        creds_no_refresh = GeminiOAuthCredentials(access_token="test")
        assert not creds_no_refresh.has_refresh_token()

        creds_with_refresh = GeminiOAuthCredentials(
        ...
        creds_none_refresh = GeminiOAuthCredentials(
            access_token="test", refresh_token=None
        )
