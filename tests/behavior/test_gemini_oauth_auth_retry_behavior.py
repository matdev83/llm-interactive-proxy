...
class TestAuthRetryBehavior:
    ...
    def test_logging_provides_visibility(self) -> None:
        ...
        assert "Received 401" in source, \
            "401 detection should be logged"
...
class TestAuthRetryArchitecture:
    ...
    def test_retry_is_recursive_call(self) -> None:
        ...
        # Streaming path uses stream_generator recursion
        assert "stream_generator(" in source, \
            "Streaming should use stream_generator recursion"
        
        # Non-streaming uses method recursion
        assert "_chat_completions_code_assist(" in source, \
    ...
    def test_response_is_passed_through_on_retry_success(self) -> None:
        ...
        # Streaming yields chunks from retry
        assert "yield retry_chunk" in source or "async for retry_chunk" in source, \
            "Streaming should yield chunks from retry"
