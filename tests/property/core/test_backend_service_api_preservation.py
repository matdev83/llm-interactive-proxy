...
class TestBackendServiceAPIPreservation:
    ...
    def test_method_signatures_match_interface(self) -> None:
        ...
        interface_methods = {
            name: method
            for name, method in inspect.getmembers(IBackendService, predicate=inspect.isfunction)
            if not name.startswith("_")
        }

        implementation_methods = {
            name: method
            for name, method in inspect.getmembers(BackendService, predicate=inspect.isfunction)
            if not name.startswith("_")
        }
        ...
        for name, interface_method in interface_methods.items():
            ...
            assert len(interface_params) == len(impl_params), f"Parameter count mismatch for {name}"
            
            for i_param, impl_param in zip(interface_params, impl_params):
                assert i_param.name == impl_param.name, f"Parameter name mismatch in {name}: {i_param.name} vs {impl_param.name}"
                assert i_param.kind == impl_param.kind, f"Parameter kind mismatch in {name}: {i_param.name}"
            ...
            if "return" in interface_hints:
                assert "return" in impl_hints, f"Missing return type hint in implementation of {name}"
    ...
    def test_legacy_helpers_exist(self) -> None:
        ...
        for method_name in legacy_methods:
            assert hasattr(BackendService, method_name), f"Missing legacy helper {method_name}"
