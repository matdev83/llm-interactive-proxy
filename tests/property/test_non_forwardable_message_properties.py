...
def message_role_strategy(draw: Any) -> str:
    ...
    return draw(
        st.sampled_from(["user", "assistant", "system", "tool", "model"])
    )
...
@st.composite
def message_content_strategy(draw: Any) -> str | list[MessageContentPartText | MessageContentPartImage] | None:
...
def test_property_identity_determinism(message: ChatMessage) -> None:
    ...
    *For any* ChatMessage, computing identity multiple times SHALL produce
    ...
    assert all(c in "0123456789abcdef" for c in identity1), "Identity must be hexadecimal"
    ...
    # Note: The identity service normalizes line endings, so CRLF/CR/LF all become LF
    if isinstance(message.content, str) and message.content and "\r" not in message.content:
        # Only test if original doesn't already have CR characters
        # Create message with CRLF instead of LF (simple case)
        content_crlf = message.content.replace("\n", "\r\n")
        message_crlf = message.model_copy()
        message_crlf.content = content_crlf
        identity_crlf = service.compute_identity(message_crlf)
        # The identity service normalizes line endings, so these should match
        assert (
            identity1 == identity_crlf
        ), f"Messages differing only in line endings must have same identity. Original: {identity1[:16]}..., CRLF: {identity_crlf[:16]}..."
...
def test_property_tool_result_compaction_stability(
    ...
    *For any* tool result message (role="tool" with tool_call_id), the identity
    ...
    assert identity1 != identity3, "Different tool_call_id must produce different identity"
...
async def test_property_filtering_order_preservation(
    ...
    *For any* message list, after filtering non-forwardable messages, the relative
    order of remaining forwardable messages SHALL be preserved.
    """
    ...
    forwardable_indices = [i for i in range(len(messages)) if i not in tagged_indices]
    assert len(filtered) == len(forwardable_indices), "Filtered count must match forwardable messages"
    
    # Check that filtered messages are in the same relative order
    filtered_index = 0
    for original_index in forwardable_indices:
    ...
    # Verify filtered count matches number of tagged message indices
    # Note: filtered_count counts messages removed, which equals tagged_indices count
    expected_filtered_count = len(tagged_indices)
    assert filtered_count == expected_filtered_count, f"Filtered count ({filtered_count}) must match tagged message count ({expected_filtered_count})"
...
async def test_property_filtering_removes_only_tagged_messages(
    ...
    *For any* message list and tag configuration, all removed messages SHALL
    have identities that are tagged in the registry for the session and scope.
    No untagged messages SHALL be removed.
    """
    identity_service = NonForwardableMessageIdentityService()
    ...
    assert (
        removed_identities.issubset(tagged_identities)
    ...
    # Verify filtered count matches number of tagged message indices
    # Note: filtered_count counts messages removed, which equals tagged_indices count
    # (not unique identities, since duplicate identities can result in multiple messages being filtered)
    expected_filtered_count = len(tagged_indices)
    assert filtered_count == expected_filtered_count, f"Filtered count ({filtered_count}) must match tagged message count ({expected_filtered_count})"
...
async def test_property_filtering_no_content_mutation(
    ...
    *For any* message list, after filtering non-forwardable messages, the
    ...
    original_forwardable_identities = {
        identities[i] for i in range(len(messages)) if identities[i] not in tagged_identities
    }
    ...
    for filtered_msg in filtered:
        ...
        original_msg = next(
            msg for msg, ident in zip(messages, identities) if ident == filtered_identity
        )
...
async def test_property_filtering_scope_semantics(
    ...
    *For any* message list with injected boundary:
    ...
    injected_tagged_indices = set(
        range(len(client_messages), len(all_messages), 2)
    ) if injected_messages else set()
    injected_tagged_identities = {
        identities[i] for i in injected_tagged_indices
    }
    ...
    for idx in client_tagged_indices:
        ...
        assert (
            msg_identity not in {identity_service.compute_identity(msg) for msg in filtered}
    ...
    untagged_injected_indices = set(range(len(client_messages), len(all_messages))) - injected_tagged_indices
    for idx in untagged_injected_indices:
        msg_identity = identities[idx]
        assert (
            msg_identity in {identity_service.compute_identity(msg) for msg in filtered}
