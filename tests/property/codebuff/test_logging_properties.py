...
def test_property_22_connection_logging(session_id):
    ...
    with patch.object(logging.getLogger("src.codebuff.connection_manager"), "info") as mock_log:
        ...
        for call in mock_log.call_args_list:
            ...
            # Check if "session_id" is in the format string
            if len(args) > 0 and "session_id" in args[0]:
                # Check if the actual session_id value is in the arguments
    ...
    with patch.object(logging.getLogger("src.codebuff.connection_manager"), "error") as mock_log:
        with patch.object(logging.getLogger("src.codebuff.connection_manager"), "warning") as mock_warning:
            ...
            for call in all_calls:
                ...
                if len(args) > 0 and "session" in args[0].lower():
                    # Check if the actual session_id value is in the arguments
                    if len(args) > 1 and args[1] == session_id:
                        logged_session_id = True
                        break
...
def test_property_25_disconnect_logging(session_id):
    ...
    with patch.object(logging.getLogger("src.codebuff.connection_manager"), "info") as mock_log:
        ...
        for call in mock_log.call_args_list:
            args, kwargs = call[0], call[1]
            # Check if "disconnect" is in the format string
            if len(args) > 0 and "disconnect" in args[0].lower():
                # Check if the actual session_id value is in the arguments
                if len(args) > 1 and args[1] == session_id:
                    logged_session_id = True
                    break
