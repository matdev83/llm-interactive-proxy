...
class TestQwenOAuthInteractiveCommandsIntegration:
    ...
    def test_backend_object_accessibility_debug(self, client):
        ...
        if has_attr:
            ...
            print(f"Backend functional: {backend_obj.is_functional if backend_obj else 'N/A'}")
        ...
        functional_backends = getattr(app.state, 'functional_backends', set())
    ...
    def test_model_setting_with_real_credentials(self, client):
        ...
        payload1 = {
            ...
            "messages": [
                {
                    "role": "user", 
                    "content": "!/set(backend=qwen-oauth)"
                }
            ],
            "max_tokens": 5,
            "temperature": 0.1,
        ...
        payload2 = {
            ...
            "messages": [
                {
                    "role": "user", 
                    "content": "!/set(model=qwen-oauth:qwen3-coder-plus)"
                }
            ],
            "max_tokens": 5,
            "temperature": 0.1,
    ...
    def test_direct_qwen_oauth_model_request(self, client):
        ...
        if response.status_code == 200:
            ...
            print("⚠️ Direct qwen-oauth request failed with auth error (expected without credentials)")
        else:
            print(f"Direct qwen-oauth request failed with status {response.status_code}: {response.text}")
