...
async def test_request_counter_incremented_on_request(
    ...
    assert gemini_oauth_plan_connector._request_counter is not None
    gemini_oauth_plan_connector._request_counter.persistence_path = persistence_path
    gemini_oauth_plan_connector._request_counter.count = 0
    gemini_oauth_plan_connector._request_counter._save_state()

    # Set mock credentials to prevent AuthenticationError
    gemini_oauth_plan_connector._oauth_credentials = {"access_token": "test-token"}
    gemini_oauth_plan_connector.gemini_api_base_url = "https://mock.googleapis.com"
    ...
    with (
        ...
        patch.object(
            gemini_oauth_plan_connector.translation_service,
            "to_domain_request",
        ...
        await gemini_oauth_plan_connector.chat_completions(
            request_data,
    ...
    assert gemini_oauth_plan_connector._request_counter.count == 1
