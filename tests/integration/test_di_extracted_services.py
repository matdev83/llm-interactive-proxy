...
from src.core.di.services import register_core_services, _ensure_tool_call_reactor_services
...
from src.core.interfaces.uri_parameter_applicator_interface import IURIParameterApplicator
from src.core.interfaces.reasoning_config_applicator_interface import IReasoningConfigApplicator
from src.core.interfaces.planning_phase_manager_interface import IPlanningPhaseManager
from src.core.interfaces.backend_lifecycle_manager_interface import IBackendLifecycleManager
...
class TestDIIntegration:
    ...
    def test_backend_service_injection(self):
        ...
        # Register dependencies required by some services
        collection.add_instance(BackendFactory, MagicMock(spec=BackendFactory))
        collection.add_instance(IBackendConfigProvider, MagicMock(spec=IBackendConfigProvider))
        collection.add_instance(IWireCapture, MagicMock(spec=IWireCapture))
        collection.add_instance(BackendRoutingService, MagicMock(spec=BackendRoutingService))
        collection.add_instance(ResilienceCoordinator, MagicMock(spec=ResilienceCoordinator))
