...
class TestQwenOAuthProxyIntegration:
    ...
    def test_qwen_oauth_chat_completion_via_proxy(self, qwen_oauth_client):
        ...
        request_payload = {
            ...
            "messages": [
                {"role": "user", "content": "Respond with exactly 'Proxy test successful' and nothing else."}
            ],
            "max_tokens": 10,
            "temperature": 0.1,
            "stream": False
...
def test_qwen_oauth_credentials_available():
    ...
    if QWEN_OAUTH_AVAILABLE:
        ...
        try:
            ...
            print(f"   - Has access token: {'✅' if creds.get('access_token') else '❌'}")
            print(f"   - Has refresh token: {'✅' if creds.get('refresh_token') else '❌'}")
            ...
            if creds.get('expiry_date'):
                ...
                if expiry_time > current_time:
                    ...
                    print(f"   - Token expired: {int(current_time - expiry_time)} seconds ago")
                    
        except Exception as e:
            print(f"⚠️  Could not read credential details: {e}")
    else:
        pytest.skip("Qwen OAuth credentials not available. Run 'qwen-code --auth' to authenticate.")
