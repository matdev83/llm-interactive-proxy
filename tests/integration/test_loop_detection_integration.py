...
class TestLoopDetectionIntegration:
    ...
    async def test_streaming_response_loop_detection(self):
        ...
        config = LoopDetectionConfig(
            enabled=True, buffer_size=8192, max_pattern_length=8192
        )
        ...
        async for chunk in processed_response.body_iterator:
            ...
            if "Response cancelled" in str(chunk):
                break

        full_content = "".join(str(chunk) for chunk in content_chunks)
    ...
    async def test_non_streaming_response_loop_detection(self):
        ...
        config = LoopDetectionConfig(
            enabled=True, buffer_size=8192, max_pattern_length=1000
        )
        config.short_pattern_threshold.min_repetitions = 3
        config.short_pattern_threshold.min_total_length = 10
        ...
        processed_response = await middleware.process_response(
            response_with_loop, context
        )

        # Should contain loop detection notice
        if isinstance(processed_response, dict) and "choices" in processed_response:
    ...
    def test_whitelist_patterns_respected(self):
        ...
        config = LoopDetectionConfig(
            enabled=True, whitelist=["...", "---", "TEST_PATTERN"]
        )
