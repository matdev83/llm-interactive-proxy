...
class TestLoopDetectionSimpleIntegration:
    ...
    def test_whitelist_functionality(self):
        ...
        from src.loop_detection.patterns import PatternAnalyzer
        analyzer = PatternAnalyzer(whitelist=config.whitelist)
        
        # Test whitelist detection
        assert analyzer.is_whitelisted("...")
        assert analyzer.is_whitelisted("---")
        assert analyzer.is_whitelisted("===")
        assert not analyzer.is_whitelisted("ERROR")
