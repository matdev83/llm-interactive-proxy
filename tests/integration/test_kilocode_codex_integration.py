...
class TestKiloCodeCodexIntegration:
    ...
    def test_kilocode_xml_parsing_completion_markers(self):
        ...
        for xml, expected_name in zip(xml_variants, expected_names, strict=False):
            result = parse_textual_tool_invocation(xml)
            assert result is not None, f"Failed to parse: {xml}"
    ...
    async def test_kilocode_tool_execution_read_file(self, codex_connector):
        ...
        result = await codex_connector._execute_kilocode_tool(
    ...
    async def test_kilocode_tool_execution_completion_marker(self, codex_connector):
        ...
        result = await codex_connector._execute_kilocode_tool(
            "completion_marker", {"result": "Successfully created the test project"}
        )
    ...
    async def test_kilocode_tool_execution_followup_marker(self, codex_connector):
        ...
        result = await codex_connector._execute_kilocode_tool(
            "followup_marker", {"question": "Would you like me to add more tests?"}
        )
    ...
    async def test_end_to_end_kilocode_workflow(self, codex_connector):
        ...
        list_result = await codex_connector._execute_kilocode_tool(
        ...
        read_result = await codex_connector._execute_kilocode_tool(
        ...
        search_result = await codex_connector._execute_kilocode_tool(
            "grep_files", {"pattern": "def ", "path": "src"}
        )
        assert search_result["exit_code"] == 0
        assert "main.py" in search_result["output"]

        # 4. Mark completion
        completion_result = await codex_connector._execute_kilocode_tool(
            "completion_marker", {"result": "Project analysis completed"}
        )
