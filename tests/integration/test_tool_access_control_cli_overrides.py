...
class TestToolAccessControlCLIOverrides:
    ...
    def test_cli_allowed_tools_override(self):
        ...
        is_allowed, metadata = policy_service.is_tool_allowed("read_file", "test:model")
    ...
    def test_cli_default_policy_override(self):
        ...
        is_allowed, metadata = policy_service.is_tool_allowed("any_tool", "test:model")
        assert is_allowed is False
        assert metadata["policy_applied"] == "global_override"
    ...
    def test_cli_combined_allowed_and_blocked_patterns(self):
        ...
        is_allowed, _ = policy_service.is_tool_allowed("write_file", "test:model")
        assert is_allowed is True
        ...
        is_allowed, _ = policy_service.is_tool_allowed("delete_file", "test:model")
        assert is_allowed is False

        # Tool not in allowed or blocked should be blocked by default deny
        is_allowed, _ = policy_service.is_tool_allowed("execute_command", "test:model")
        assert is_allowed is False
    ...
    def test_cli_override_with_regex_patterns(self):
        ...
        assert policy_service.is_tool_allowed("read_file", "test:model")[0] is True
        assert policy_service.is_tool_allowed("list_directory", "test:model")[0] is True
        assert policy_service.is_tool_allowed("get_data", "test:model")[0] is True

        # Blocked patterns should be blocked
        assert policy_service.is_tool_allowed("delete_all", "test:model")[0] is False
        assert policy_service.is_tool_allowed("bulk_delete", "test:model")[0] is False

        # Not matching any pattern with default deny
        assert policy_service.is_tool_allowed("write_file", "test:model")[0] is False
