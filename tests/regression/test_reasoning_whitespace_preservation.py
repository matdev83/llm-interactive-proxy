...
def connector():
    ...
    c = KimiCodeConnector(client=client, config=config, translation_service=translation_service)
    c.api_key = "test-key"
    c._prepare_payload = AsyncMock(return_value={"model": "kimi-for-coding", "messages": []})
    return c
...
async def test_regression_kimi_reasoning_delta_conversion_all_aliases(connector):
    ...
    request = CanonicalChatRequest(model="kimi/kimi-for-coding", messages=[ChatMessage(role="user", content="hi")])
...
async def test_regression_accumulation_processor_preserves_whitespace():
    ...
    chunk1 = StreamingContent(content={"choices": [{"delta": {"reasoning_content": "Line"}}]}, stream_id="test")
    chunk2 = StreamingContent(content={"choices": [{"delta": {"reasoning_content": " 1"}}]}, stream_id="test", is_done=True)
