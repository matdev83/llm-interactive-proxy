...
def test_record_concurrent_thread_safe():
    ...
    threads = [threading.Thread(target=record_params, args=(i,)) for i in range(num_threads)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()
    ...
    assert len(pr._history) <= pr._MAX_HISTORY_SIZE, \
...
def test_size_limit_enforcement_thread_safe():
    ...
    assert len(pr._history) <= pr._MAX_HISTORY_SIZE * 2, \
