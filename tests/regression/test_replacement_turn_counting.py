...
async def test_replacement_cycle_respects_turn_count(mock_backend_registry):
    ...
    # Turn 1
    assert service.should_replace(session_id, RequestContext(headers={}, cookies={}, state={}, app_state=None))
    backend, model = service.get_effective_backend_model(session_id, "original", "model")
    ...
    backend, model = service.get_effective_backend_model(session_id, "original-backend", "original-model")
