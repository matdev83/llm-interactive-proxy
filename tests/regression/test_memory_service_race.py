...
async def test_analysis_progress_concurrent_access():
    ...
    tasks = [asyncio.create_task(service.complete_analysis(session_id)) for _ in range(5)]
...
async def test_queue_operations_concurrent():
    ...
    assert len(set(all_consumed)) == len(all_consumed), "No session should be consumed twice"
