...
async def test_concurrent_mark_operations_no_race():
    ...
    session_ids = [f"session-{i}" for i in range(20)]  # Reduced from 30
    ...
    tasks = [mark_dirty_batch(session_ids) for _ in range(10)] + [  # Reduced from 15
        mark_clean_batch(session_ids) for _ in range(10)  # Reduced from 15
    ]
