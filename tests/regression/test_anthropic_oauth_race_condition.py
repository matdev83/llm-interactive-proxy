...
async def test_anthropic_oauth_reload_lock_exists():
    ...
    assert isinstance(
        backend._reload_lock, asyncio.Lock
    ), "_reload_lock should be an asyncio.Lock"
...
async def test_anthropic_oauth_reload_lock_protects_concurrent_access():
    ...
    results = await asyncio.gather(*[simulate_reload() for _ in range(5)])
...
async def test_anthropic_oauth_reload_prevents_duplicate_tasks():
    ...
    assert (
        backend._load_oauth_credentials.call_count == 0
    ), "No reload should be scheduled when task is pending"
