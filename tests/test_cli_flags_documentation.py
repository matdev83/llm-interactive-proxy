...
def test_cli_flags_documented(request):
    ...
    if missing_flags:
        ...
        request.config.cache.set(hash_key, current_hash) # Still update hash to know which version failed
        
        pytest.fail(
            f"The following CLI flags are missing from documentation in ./docs/:\n" + 
