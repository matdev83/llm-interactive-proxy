...
class ValidatedTestStage(ABC):
    ...
    def register_service(self, name: str, service: Any, force_sync: bool = False) -> None:
    ...
    def _validate_service(self, service: Any, name: str, force_sync: bool = False) -> Any:
        ...
        if name.lower() in ['session', 'session_service'] and not isinstance(service, SafeSessionService):
            warnings.warn(
    ...
    def _should_be_sync_service(self, service_name: str) -> bool:
        ...
        sync_patterns = [
            'session', 'config', 'registry', 'cache', 'validator', 
            'parser', 'formatter', 'logger', 'metrics'
        ]
...
class RealBackendTestStage(ValidatedTestStage):
    ...
    def setup(self) -> None:
        ...
        self.register_service(
            'session_service',
            SafeSessionService({'test_mode': True}),
            force_sync=True
        )
    ...
    def check_for_unawaited_coroutines(obj: Any) -> list[str]:
        ...
        if hasattr(obj, '__dict__'):
            for attr_name, attr_value in obj.__dict__.items():
                ...
                elif isinstance(attr_value, AsyncMock) and not asyncio.iscoroutinefunction(getattr(obj, attr_name, None)):
                    warnings_found.append(
