...
async def test_backend_service_isolation():
    ...
    backend_a = await backend_service._get_or_create_backend("openai", session_id="session_A")
    
    # 2. Request backend for Session B
    backend_b = await backend_service._get_or_create_backend("openai", session_id="session_B")
    
    # 3. Verify they are DIFFERENT instances
    assert backend_a is not backend_b, "Backends should be different instances for different sessions"
