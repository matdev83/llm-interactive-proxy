...
def create_test_messages(num_messages: int = 2) -> list[dict[str, Any]]:
    ...
    # Add user message
    messages.append({"role": "user", "content": "Hello, this is a test message."})
...
def create_streaming_response_chunks(
    ...
    # Create a chunk for each part with varied structure to avoid loop detection
    for i, content_part in enumerate(content_chunks):
        chunk: dict[str, Any] = {
            "id": response_id,
            "object": "chat.completion.chunk",
            "created": created + i,  # Vary timestamp to avoid loop detection
            "model": model,
            "choices": [
                {
                    "index": 0,
...
def mock_backend_api(
    ...
    """
    # Create streaming response chunks as raw bytes
    ...
    def dynamic_handler(request: httpx.Request) -> httpx.Response:
        ...
        if isinstance(payload_json, dict) and payload_json.get("stream") is True:
            # Return streaming response with proper format
            return httpx.Response(
                200, headers={"Content-Type": "text/event-stream"}, content=stream_data
            )
        # Non-streaming fallback
        return httpx.Response(200, json=create_chat_response_json())

    # Register a single route for both streaming and non-streaming cases
    respx_mock.post(f"{base_url}/chat/completions").mock(side_effect=dynamic_handler)
