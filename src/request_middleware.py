    ...
    async def process_request(
        ...
        for processor in self.middleware_stack:
            if processor.should_process(processed_messages, request_context):
                processed_messages = await processor.process(processed_messages, request_context)
...
class RedactionProcessor(RequestProcessor):
    ...
    def should_process(
        ...
        return context.redaction_enabled and (context.api_key_redactor is not None or context.command_filter is not None)
            ...
            if "content" in processed_message:
                ...
                if content is not None:
                    ...
                    if context.api_key_redactor is not None:
                        content = self._redact_message_content(content, context.api_key_redactor)
                    
                    # Apply command filtering second
                    if context.command_filter is not None:
                        content = self._filter_message_content(content, context.command_filter)
...
def configure_redaction_middleware():
