...
class AsyncSyncPatternChecker:
    ...
    def check_directory(self, directory: Path, pattern: str = "test_*.py") -> dict[str, list[str]]:
        """
    ...
    def _check_class_def(self, node: ast.ClassDef, filename: str) -> None:
        ...
        if (any(base.id == 'InitializationStage' for base in node.bases if isinstance(base, ast.Name)) and
            ('TestStage' in node.name or 'MockStage' in node.name)):
    ...
    def _check_assignment(self, node: ast.Assign, filename: str) -> None:
        ...
        if (isinstance(node.value, ast.Call) and
            isinstance(node.value.func, ast.Name) and node.value.func.id == 'AsyncMock'):
                # Check if this is for a session service or similar
                for target in node.targets:
                    if (isinstance(target, ast.Name) and
                        'session' in target.id.lower()):
                            self.issues.append(
    ...
    def _check_function_call(self, node: ast.Call, filename: str) -> None:
        ...
        if (isinstance(node.func, ast.Attribute) and node.func.attr == 'add_instance' and
            len(node.args) >= 2 and
            isinstance(node.args[1], ast.Call) and
            isinstance(node.args[1].func, ast.Name) and node.args[1].func.id == 'AsyncMock'):
                        self.issues.append(
...
class RuntimePatternChecker:
    ...
    def check_service_registration(service_type: type, instance: Any) -> list[str]:
        ...
        if (hasattr(service_type, '__name__') and 'Session' in service_type.__name__ and
            hasattr(instance, 'get_session')):
                try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pattern", default="test_*.py", help="File pattern for directory scanning")
