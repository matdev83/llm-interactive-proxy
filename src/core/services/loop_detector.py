...
class LoopDetector(ILoopDetector):
    ...
    async def check_for_loops(self, content: str) -> LoopDetectionResult:
        ...
        def _as_int(value: Any, default: int) -> int:
            try:
                ...
                if isinstance(value, dict):
                    # Try common keys
                    for k in ("min_pattern_length", "max_pattern_length", "min_repetitions", "value"):
                        if k in value and isinstance(value[k], int):
                            return int(value[k])
                # Try attribute access
                for attr in ("min_pattern_length", "max_pattern_length", "min_repetitions", "value"):
        ...
        try:
            ...
            config = LoopDetectionConfig(
                ...
                content_loop_threshold=min_reps
            )
            ...
            if results and results[0].repetitions >= min_reps:
                ...
                return LoopDetectionResult(
                    ...
                    details={
                        ...
                        "total_repeated_chars": len(result.pattern) * result.repetitions if result.pattern else 0,
                    }
                )
    ...
    async def configure(self, 
        ...
        logger.debug(f"Loop detector configuration updated: min_pattern_length={min_pattern_length}, max_pattern_length={max_pattern_length}, min_repetitions={min_repetitions}")
...
def create_loop_detector(min_pattern_length: int = 50,
                        max_pattern_length: int = 500,
                        min_repetitions: int = 2,
                        max_samples: int = 20) -> LoopDetector:
