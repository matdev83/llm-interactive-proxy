    ...
    def record_lock_contention(self, wait_time: float, lock_name: str):
        ...
        with self._metrics_lock:
            ...
            if wait_time > 1.0:  # 1 second wait
                logger.warning(f"High lock contention detected in {lock_name}: {wait_time:.3f}s wait time")
    ...
    async def acquire(self):
        ...
        try:
            ...
            if wait_time > 5.0:  # 5 second threshold
                logger.warning(f"Long lock wait detected for {self.name}: {wait_time:.3f}s")
                
        except asyncio.TimeoutError:
            production_metrics.record_deadlock_detection(self.name)
            raise Exception(f"Potential deadlock detected in lock {self.name} after {self.timeout}s")
    ...
    def release(self):
        ...
        if self._acquired_at:
            ...
            if hold_time > 10.0:  # 10 second threshold
                logger.warning(f"Long lock hold detected for {self.name}: {hold_time:.3f}s")
        ...
        async def async_wrapper(*args, **kwargs) -> T:
            ...
            for attempt in range(1, config.max_attempts + 1):
                try:
                    ...
                    logger.info(f"Retrying {func.__name__} in {delay:.3f}s (attempt {attempt}/{config.max_attempts})")
...
class ConcurrencyGuard:
    ...
    async def acquire(self, operation_name: str = "unknown"):
        ...
        if len(self._active_operations) >= self.max_concurrent:
            ...
            production_metrics.record_race_condition_warning(f"{self.name}:{operation_name}")
        ...
        async with self._semaphore:
            ...
            try:
                ...
                if duration > 30.0:  # 30 second threshold
                    logger.warning(f"Long operation in {self.name}: {operation_name} took {duration:.3f}s")
...
def get_production_metrics() -> Dict[str, Any]:
    ...
    with production_metrics._metrics_lock:
        avg_wait_time = sum(production_metrics.lock_wait_times) / len(production_metrics.lock_wait_times) if production_metrics.lock_wait_times else 0
        max_wait_time = max(production_metrics.lock_wait_times) if production_metrics.lock_wait_times else 0
