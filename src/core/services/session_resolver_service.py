    ...
    def __init__(self, config: IConfig | None = None) -> None:
        ...
        if config is not None:
            try:
                ...
                if hasattr(config, "session") and hasattr(config.session, "default_session_id"):
                    configured_default = getattr(config.session, "default_session_id")
    ...
    async def resolve_session_id(self, context: RequestContext) -> str:
        ...
        if hasattr(context, "domain_request"):
            ...
            if domain_request is not None:
                session_id = getattr(domain_request, "session_id", None)
                if session_id:
                    return session_id
        ...
        session_id = context.cookies.get("session_id")
        if session_id:
            return session_id
