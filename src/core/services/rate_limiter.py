...
class InMemoryRateLimiter(IRateLimiter):
    ...
    def __init__(self, default_limit: int = 60, default_time_window: int = 60):
        ...
        logger.info(f"Initialized InMemoryRateLimiter with defaults: {default_limit}/{default_time_window}s")
    ...
    async def check_limit(self, key: str) -> RateLimitInfo:
        ...
        logger.debug(f"Rate limit check: {key} - {used}/{limit} used, limited: {is_limited}")
...
def create_rate_limiter(config: dict[str, Any]) -> IRateLimiter:
    ...
    default_time_window = config.get("default_rate_window", 60)
