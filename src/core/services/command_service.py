...
class CommandService(ICommandService):
    ...
    async def process_commands(self, messages: list[Any], session_id: str) -> ProcessedResult:
        """
        ...
        """
        if not messages:
            return ProcessedResult(modified_messages=[], command_executed=False, command_results=[])
            
        # Get the session
        session = await self._session_service.get_session(session_id) if self._session_service else None
                    ...
                    if match:
                        ...
                        if cmd:
                            ...
                            if asyncio.iscoroutinefunction(cmd.execute):
                                ...
                                result = cmd.execute(args, session, None)
                            ...
                            # Remove unknown command
                                                            # Remove unknown command
                                content_without_cmd = content.replace(f"!/{cmd_name}", "", 1)
                                if args_str:
                                    content_without_cmd = content_without_cmd.replace(f"({args_str})", "", 1)
                                modified_messages[i]["content"] = " " + content_without_cmd.strip()
                break
                
        return ProcessedResult(modified_messages=modified_messages, command_executed=command_executed, command_results=command_results)
