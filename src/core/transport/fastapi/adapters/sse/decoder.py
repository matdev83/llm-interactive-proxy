...
class SSEDecoder:
    ...
    def decode_payload(self, payload: bytes | str) -> DecodedSSE:
        ...
        try:
            ...
            return DecodedSSE(
                content=data_body, metadata=metadata_hint, is_done=forced_done
            )
        except (ValueError, RecursionError):
            # Security: Handle potential DoS attempts
            return DecodedSSE(
                content=data_body,
                metadata={"error": "invalid_json_structure"},
                is_done=forced_done,
            )
        ...
        return DecodedSSE(content=decoded, metadata=metadata_hint, is_done=forced_done)
