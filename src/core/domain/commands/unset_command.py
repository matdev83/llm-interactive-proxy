...
class UnsetCommand(BaseCommand):
    ...
    async def execute(
        ...
        if success_count == 0:
            return CommandResult(success=False, message="unset: nothing to do", name=self.name)
    ...
    def _unset_backend(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        new_backend_config = state.backend_config.without_override()
        updated_state = state.with_backend_config(new_backend_config)
        result = CommandResult(success=True, message="Backend reset to default", data={"backend": None})
        return result, updated_state

    def _unset_model(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        new_backend_config = state.backend_config.with_model(None)
        updated_state = state.with_backend_config(new_backend_config)
        result = CommandResult(success=True, message="Model reset to default", data={"model": None})
        return result, updated_state

    def _unset_temperature(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        ...
        result = CommandResult(success=True, message=f"Temperature reset to default ({default_temp})", data={"temperature": default_temp})
        return result, updated_state

    def _unset_interactive_mode(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        new_backend_config = state.backend_config.with_interactive_mode(True)
        updated_state = state.with_backend_config(new_backend_config).with_interactive_just_enabled(False)
        result = CommandResult(success=True, message="Interactive mode reset to default (enabled)", data={"interactive-mode": True})
        return result, updated_state

    def _unset_redact_api_keys(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        ...
        if service_provider:
            try:
                ...
                if app_settings:
                    ...
                    result = CommandResult(success=True, message="API key redaction reset to default (enabled)", data={"redact-api-keys-in-prompts": True})
    ...
    def _unset_command_prefix(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        # Try to get the app settings service from the service provider
        service_provider = context.get("service_provider")
        if service_provider:
            try:
                ...
                if app_settings:
                    ...
                    result = CommandResult(success=True, message="Command prefix reset to default (!/)", data={"command-prefix": "!/"})
                    return result, state  # No state change in session
            except Exception:
                pass
    ...
    def _unset_project(self, state: ISessionState, context: Any) -> tuple[CommandResult, ISessionState]:
        updated_state = state.with_project(None)
        result = CommandResult(success=True, message="Project reset to default", data={"project": None})
        return result, updated_state