...
class StreamingContent:
    ...
    def from_raw(cls, data: Any) -> StreamingContent:
        ...
        # Fall back to empty content for unknown types
            ...
            logger.warning(f"Unhandled data type in StreamingContent.from_raw: {type(data)}")
            return cls(content="", metadata={"unknown_type": str(type(data))}, raw_data=data)
...
class StreamNormalizer:
    ...
    def __init__(self, processors: Optional[List[IStreamProcessor]] = None) -> None:
    ...
    ) -> AsyncIterator[Union[StreamingContent, bytes]]:
...
class LoopDetectionProcessor(IStreamProcessor):
    ...
    def __init__(self, loop_detector: Any, accumulated_threshold: int = 100) -> None:
        """Initialize the loop detection processor.
        
        Args:
            loop_detector: The loop detector service to use
            accumulated_threshold: Minimum content length before checking for loops
        """
        self.loop_detector = loop_detector
        self.accumulated_threshold = accumulated_threshold
    ...
    async def process(self, content: StreamingContent) -> StreamingContent:
        ...
        Raises:
            LoopDetectionError: If a repetitive pattern is detected
        """
        from src.core.common.exceptions import LoopDetectionError
        
        # Skip loop detection for done markers
        if content.is_done:
            return content
        
        # Accumulate content for loop detection
        if content.content:
            self.accumulated_content += content.content
            
            # Check for loops if we have enough content
            if self.loop_detector and len(self.accumulated_content) > self.accumulated_threshold:
                loop_result = await self.loop_detector.check_for_loops(self.accumulated_content)
