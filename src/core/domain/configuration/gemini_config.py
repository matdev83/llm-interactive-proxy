...
class GeminiGenerationConfig(ValueObject, IBackendSpecificConfig):
    ...
    def with_generation_config(self, config_json: str | dict[str, Any]) -> GeminiGenerationConfig:
        ...
        if "thinkingConfig" in config_dict and isinstance(config_dict["thinkingConfig"], dict):
            updates["thinking_config"] = config_dict["thinkingConfig"]
            
        # Handle safety settings
        if "safetySettings" in config_dict and isinstance(config_dict["safetySettings"], dict):
            updates["safety_settings"] = config_dict["safetySettings"]
            
        # Handle generation parameters
        for param in ["temperature", "top_p", "top_k", "max_output_tokens", "candidate_count", "stop_sequences"]:
