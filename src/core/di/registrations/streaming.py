...
def register(services: ServiceCollection, app_config: AppConfig | None) -> None:
    ...
    # Register session cancellation coordinator (must be before StreamNormalizer)
    _register_session_cancellation_coordinator(services)

    # Register client termination reason mapper
    _register_client_termination_reason_mapper(services)

    # Register client end-of-session service
    _register_client_end_of_session_service(services)

    # Register session cancellation cleanup subscriber
    _register_session_cancellation_cleanup_subscriber(services)
