...
def _register_tool_call_reactor_core(
    ...
    def history_tracker_factory(
        ...
        if config is not None and hasattr(config.session, "tool_call_reactor"):
            ...
            session_ttl = getattr(reactor_config, "session_ttl_seconds", session_ttl)
...
def _register_tool_call_reactor_orchestrator(
    ...
    def orchestrator_factory(
        ...
        stream_context_resolver: IToolCallStreamContextResolver = (
            provider.get_required_service(cast(type, IToolCallStreamContextResolver))
        )
...
def _register_json_repair_service(
    ...
    """Register JSON repair service."""
    from src.core.services.json_repair_service import JsonRepairService

    # JsonRepairService is always available (no config needed)
    register_singleton_if_absent(services, JsonRepairService)

    if logger.isEnabledFor(logging.DEBUG):
        logger.debug("Registered JsonRepairService")
