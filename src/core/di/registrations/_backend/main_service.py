...
def register_backend_service(services: ServiceCollection) -> None:
    ...
    def _backend_service_factory(provider: IServiceProvider) -> BackendService:
        ...
        failover_coordinator = provider.get_service(cast(type, IFailoverCoordinator))
        ...
        usage_tracking_wrapper: IUsageTrackingWrapper = provider.get_required_service(
        ...
        backend_model_resolver: IBackendModelResolver = provider.get_required_service(
            cast(type, IBackendModelResolver)
        )
        failover_planner: IFailoverPlanner = provider.get_required_service(
            cast(type, IFailoverPlanner)
        )
        backend_completion_flow: IBackendCompletionFlow = provider.get_required_service(
