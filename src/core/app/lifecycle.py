...
class AppLifecycle:
    ...
    def _start_background_tasks(self) -> None:
        ...
        if self.config.get("session_cleanup_enabled", False):
            interval = self.config.get("session_cleanup_interval", 3600)  # 1 hour default
            ...
            logger.info(f"Started session cleanup task (interval: {interval}s, max age: {max_age}s)")
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if not provider:
                        logger.warning("Service provider not available for session cleanup")
                        continue
                    ...
                    if hasattr(session_service, "cleanup_expired_sessions"):
                        deleted_count = await session_service.cleanup_expired_sessions(max_age)
