...
from src.commands.base import CommandContext, CommandResult
...
class ICommandHandler(ABC):
    ...
    def convert_to_legacy_result(
        ...
        if not result.success:
            return (
                True,
                CommandResult(command_name, False, result.message),
                False,
            )
...
class BaseCommandHandler(ICommandHandler, ABC):
    ...
    def can_handle(self, param_name: str) -> bool:
        ...
        return normalized == self.name.lower() or normalized in [a.lower() for a in self.aliases]
