    ...
    def handle(
        ...
        if not isinstance(param_value, str):
            return CommandHandlerResult(
                success=False,
                message="Backend value must be a string"
            )
        ...
        if backend_val not in SUPPORTED_BACKENDS:
            return CommandHandlerResult(
                success=False,
                message=f"Backend {backend_val} not supported"
            )
        ...
        return CommandHandlerResult(
            success=True,
            message=f"Backend set to {backend_val}",
            new_state=new_state
        )
...
class ModelHandler(BaseCommandHandler):
    ...
    def handle(
        ...
        if not isinstance(param_value, str):
            return CommandHandlerResult(
                success=False,
                message="Model value must be a string"
            )
        ...
        if ":" in model_val:
            ...
            builder = SessionStateBuilder(current_state)
            new_state = builder.with_backend_type(backend_prefix).with_model(model_name).build()
            ...
            return CommandHandlerResult(
                success=True,
                message=f"Model set to {model_val}",
                new_state=new_state
            )
...
class OpenAIUrlHandler(BaseCommandHandler):
    ...
    def handle(
        ...
        if not isinstance(param_value, str):
            return CommandHandlerResult(
                success=False,
                message="OpenAI URL value must be a string"
            )
