    ...
    def handle(
        ...
        for param, value in params.items():
            ...
            if param_lower in self._handlers:
                ...
                try:
                    if param_lower == "interactive" or param_lower == "interactive-mode":
                        bool_value = self._parse_bool(str(value))
                        if bool_value is None:
                            results.append(f"Invalid boolean value for {param}: {value}")
                            success = False
                        else:
                            builder.with_backend_config(
                                current_state.backend_config.with_interactive_mode(bool_value)
                            )
                        ...
                        if effort not in ("low", "medium", "high", "maximum"):
                            results.append(f"Invalid reasoning effort: {value}. Use low, medium, high, or maximum.")
                            success = False
                        else:
                        ...
                        try:
                            ...
                            results.append(f"Invalid thinking budget: {value}. Must be an integer.")
                            success = False
                        ...
                        url = str(value)
                        builder.with_backend_config(
                            current_state.backend_config.with_openai_url(url)
                        )
                        ...
                        try:
                            repeats = int(value)
                            builder.with_loop_config(
                                current_state.loop_config.with_tool_loop_max_repeats(repeats)
                            )
                            results.append(f"Tool loop max repeats set to {repeats}")
                        except ValueError:
                            results.append(f"Invalid tool loop max repeats: {value}. Must be an integer.")
                            success = False
                        ...
                        try:
                            ...
                            results.append(f"Invalid tool loop TTL: {value}. Must be an integer.")
                            success = False
                    
                    elif param_lower == "tool-loop-mode":
                        mode = str(value).lower()
                        if mode not in ("break", "chance_then_break"):
                            results.append(f"Invalid tool loop mode: {value}. Use break or chance_then_break.")
                            success = False
                        else:
