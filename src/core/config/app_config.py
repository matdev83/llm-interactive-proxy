...
class BackendSettings(BaseModel):
    ...
    def functional_backends(self) -> set[str]:
        ...
        return {
            name for name, config in self.model_dump().items() 
            if name != "default_backend" and isinstance(config, dict) and config.get("api_key")
        }
...
class AppConfig(BaseModel):
    ...
    def to_legacy_config(self) -> dict[str, Any]:
        ...
        config = {
            ...
            # Session settings
            "session_cleanup_enabled": self.session.cleanup_enabled if self.session is not None else True,
            "session_cleanup_interval": self.session.cleanup_interval if self.session is not None else 3600,
            "session_max_age": self.session.max_age if self.session is not None else 86400,
            "default_interactive_mode": self.session.default_interactive_mode if self.session is not None else False,
            "force_set_project": self.session.force_set_project if self.session is not None else False,
            
            # Logging settings
            "log_level": self.logging.level.value if self.logging is not None else "INFO",
            "request_logging": self.logging.request_logging if self.logging is not None else True,
            "response_logging": self.logging.response_logging if self.logging is not None else False,
            "log_file": self.logging.log_file if self.logging is not None else None,
            
            # Backend settings
            "default_backend": self.backends.default_backend if self.backends is not None else "openai",
        ...
        if self.backends is not None:
            for backend, backend_config in self.backends.model_dump(exclude={"default_backend"}).items():
    ...
    def from_legacy_config(cls, legacy_config: dict[str, Any]) -> AppConfig:
        ...
        config = {
            ...
            "session": {
                ...
                "default_interactive_mode": legacy_config.get("default_interactive_mode", True),
    ...
    def from_env(cls) -> AppConfig:
        ...
        config: dict[str, object] = {
            ...
            "session": {
                "cleanup_enabled": os.environ.get("SESSION_CLEANUP_ENABLED", "true").lower() == "true",
                "cleanup_interval": int(os.environ.get("SESSION_CLEANUP_INTERVAL", "3600")),
                "max_age": int(os.environ.get("SESSION_MAX_AGE", "86400")),
                "default_interactive_mode": os.environ.get("DEFAULT_INTERACTIVE_MODE", "true").lower() == "true",
                "force_set_project": os.environ.get("FORCE_SET_PROJECT", "").lower() == "true",
            },
