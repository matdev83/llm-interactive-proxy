"""Verified installation of the latest stable go-agy-acp-wrapper release."""
...
REPOSITORY = "matdev83/go-agy-acp-wrapper"
...
def default_cache_dir() -> Path:
    override = os.environ.get("AGY_ACP_WRAPPER_CACHE_DIR", "").strip()
    if override:
    ...
    return root / "llm-interactive-proxy" / "go-agy-acp-wrapper"
...
def _cached_executables(cache_dir: Path) -> list[Path]:
    name = "go-agy-acp-wrapper.exe" if os.name == "nt" else "go-agy-acp-wrapper"
    return sorted(
...
def _extract(archive: Path, destination: Path) -> Path:
    executable_name = (
        "go-agy-acp-wrapper.exe" if archive.suffix == ".zip" else "go-agy-acp-wrapper"
    )
    ...
    async with _INSTALL_LOCK:
        try:
            ...
            installed = (
                ...
                / tag
                / (
                    "go-agy-acp-wrapper.exe"
                    if suffix.endswith(".zip")
                    else "go-agy-acp-wrapper"
                )
            )
            ...
            with tempfile.TemporaryDirectory(prefix="agy-wrapper-", dir=root) as temp:
                temp_dir = Path(temp)
