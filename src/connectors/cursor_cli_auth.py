...
class CursorAuthPolicy:
    ...
    def any_usable(self) -> bool:
        if self.cookie_usable:
            return True
        return self.env_key_present and not self.env_key_invalid
...
def probe_cursor_cli_auth(
    ...
    *,
    ...
    env = build_cursor_cli_env(mode, base=env_base)
    try:
...
def resolve_cursor_auth_policy(
    ...
    *,
    ...
    elif env_key_present and not env_key_invalid:
...
def discovery_modes_to_try(policy: CursorAuthPolicy) -> list[CursorAuthMode]:
    ...
    if policy.env_key_present and not policy.env_key_invalid:
        # list-models may require the key even when cookie status is authenticated.
