...
class ClineConnector(OpenAIConnector):
    ...
    def _discover_candidate_directories(
        ...
        # Always try the POSIX default (~/.cline)
    ...
    def _reload_from_secondary_tokens(self) -> dict[str, Any] | None:
        """Try to recover tokens from VSCode or Codex sources."""
    ...
    def _load_tokens_from_codex_auth(self) -> dict[str, Any] | None:
        """Load Codex CLI auth credentials when explicitly pointed to a secrets file."""
    ...
    def _load_tokens_from_vscode_secret_store(self) -> dict[str, Any] | None:
        """Load tokens from VSCode's secure storage (Windows only)."""
